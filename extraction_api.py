...
def get_table_stuff(case_id,document_id, tenant_id):
    ...
    logging.info(f"## DEBUG 7.1 function name : get_file_path ,\n  input : {document_id} , {parameters['ui_folder']},tenant_id \n output : {file_path}")


    wand_pdf = open_pdf(file_path)

    logging.info(f'## DEBUG 7.2 function name : open_pdf ,\n  input : {file_path} \n output : {wand_pdf}')

    tables_area = get_tables(file_path, document_id, tenant_id)
    logging.info(f'## DEBUG 7.3 function name : get_tables ,\n  input : {file_path} \n {document_id} \n {tenant_id} \n output : {tables_area}')

    try:
        table_lines = get_table_lines(tables_area, wand_pdf)
        logging.info(f'## DEBUG 7.4 function name : get_table_lines ,\n  input : {tables_area} \n {wand_pdf} \n output : {table_lines}')
...
def prepare_extraction_data(fte_datas, file_trace, fte, pre_processed_char_space, ocr_data):
    ...
    for fte_data in fte_datas:
        ...
        # logging.info(f"## DEBUG 9.1 function_name : get_match_threshold , input : false_positive={false_positive} , fte_data={fte_data}  output : match_ratio_thresholds={match_ratio_thresholds}")
        ...
        if pattern:
            ...
            value_ocr = ocr_data
        end_time=tt()
        logging.info(f" Time Consumed for get_value_ocr for {fte} is {str(end_time-strt_time)}")
        # logging.info(f"## DEBUG 9.2 function_name : get_value_ocr , input : pre_processed_char_space={pre_processed_char_space} , fte_data={fte_data}, ocr_data={ocr_data}  output : value_ocr={value_ocr}")
...
def extraction_func(fte, fte_data, fields_to_exclude, ocr_fields_dict, ocr_data, api, case_id, tenant_id,
    ...
    # logging.info(f'## DEBUG 10.1 function_name : value_only_field , input : fte_data={fte_data}, api={api}  output : val={val}, word_highlight={word_highlight}, method_used ={method_used}')
    # if val:
    #     return val, word_highlight, method_used, key_highlight
    
    
    scope, scope_value = get_scope(fte_data)
    logging.info(f'## DEBUG 10.2 function_name : get_scope  output : scope={scope}, scope_value={scope_value}')
    ...
    if context_hierarchy:
        ...
        logging.info(f'Entering if condition function_name=extract_with_hierarchy')
    ...
    if not val:
        ...
        if keyword and scope:
            ...
            try:
                ...
                logging.info(f'## DEBUG 11 function_name: extraction_with_keyword output: val={val}, word_highlight={word_highlight}, method_used={method_used}, key_highlight={key_highlight}')
            ...
            logging.info(f'## DEBUG 12 function_name scope_extracrt input: scope_value={scope_value}, scope={scope}, value_ocr={value_ocr}, '
    ...
    logging.info(f'## DEBUG 13 function_name field_confidence_checker input: word_highlight={word_highlight}, field_conf_threshold={field_conf_threshold} output: val={val}')
...
def value_extract(case_id,document_id,tenant_id,ocr_data,field_data, template_name, file_trace,fields_to_exclude,api=False,documnet_splitter=False,start_page=0):
    ...
    logging.info(f'## TE info func_name : sort_ocr -- started')        
    logging.info(f'## TE info func_name : convert_ocrs_to_char_dict_only_al_num -- started')
    ...
    logging.info(f'## TE info func_name : sort_ocr -- completed')       
    logging.info(f'## TE info func_name : convert_ocrs_to_char_dict_only_al_num -- completed')
    #use this logging statement when debug is required 
    # logging.info(f'## TE info output is (pre_processed_char,ocr_data) : {pre_processed_char,ocr_data}')
    logging.info(f'## TE info func_name : get_pre_process_char_space input (ocr_data)')       
    pre_processed_char_space = get_pre_process_char_space(ocr_data)
    logging.info(f'## TE info func_name : get_pre_process_char_space output (pre_processed_char_space)')       
    ...
    logging.info(f'## TE info func_name : get_table_stuff input (case_id,document_id, tenant_id): {case_id,document_id, tenant_id}') 
    ...
    tables_area=[]
    table_lines=[]
    logging.info(f'## TE info func_name : get_table_stuff output (file_path, wand_pdf, tables_area, table_lines): {file_path, wand_pdf, tables_area, table_lines}')       
    ...
    fields_trained = []
    file_trace={}
    # logging.info(f'## TE info func_name : get_file_trace_train input (file_trace): {file_trace}')     
    ## file_trace is not used any more - vahida
    # try:    
    #     file_trace = get_file_trace_train(file_trace)
    # except Exception as e:
    #     file_trace={}
    # logging.info(f'## TE info func_name : get_file_trace_train output (file_trace): {file_trace}')           

    logging.info(f'## TE info Entering for loop for field_data : {field_data}')           
    for fte, fte_data_to_check in field_data.items():

        logging.info(f'## TE info (fte,fte_data_to_check) for this iteration is : {fte , fte_data_to_check}')           
        ...
        logging.info(f'## TE info func_name : prepare_extraction_data input (fte_datas, file_trace, fte, pre_processed_char_space, ocr_data)')           
        combined_data = prepare_extraction_data(fte_datas, file_trace, fte, pre_processed_char_space, ocr_data)
        logging.info(f'## TE info func_name : prepare_extraction_data output (combined_data)')           

        logging.info(f'## TE info Entering for loop for combined_data')           

        for fte_data, match_ratio, extend_threshold, value_ocr in combined_data:
            logging.info(f'## TE info (fte_data, match_ratio, extend_threshold, value_ocr) for this iteration is : {fte_data, match_ratio, extend_threshold, value_ocr}')           
            val, word_highlight, method_used, key_high = (None,) * 4

            logging.info(f'## TE info func_name : extraction_func input (fte, fte_data, fields_to_exclude,ocr_fields_dict,ocr_data, api, case_id, tenant_id,file_path, wand_pdf, tables_area,table_lines,pre_processed_char, ocr_length,match_ratio,value_ocr,extend_threshold)')        
            ...
            val, word_highlight, method_used, key_high = extraction_func(fte, fte_data, fields_to_exclude,
                                                                    ...
                                                                    value_ocr=value_ocr,
                                                                    extend_threshold=extend_threshold)
            logging.info(f'## TE info func_name : extraction_func output (val, word_highlight, method_used, key_high): {val, word_highlight, method_used, key_high}')        
            ...
            if val:
                ...
                #         logging.info(f'## TE info func_name : check_multiple input (word_highlight, ocr_data): {word_highlight, ocr_data}')           
                #         output_multiple[field_multiple_name] = json.dumps(check_multiple(word_highlight, ocr_data[word_highlight['page']]))
                #         logging.info(f'## TE info func_name : check_multiple output (output_multiple): {output_multiple}')           
    ...
    logging.info(f'## TE info func_name : validate output (output_, ocr_fields_dict, standard_format): {output_, ocr_fields_dict, standard_format}')           
    output_ = validate(output_, ocr_fields_dict, standard_format)
    logging.info(f'## TE info func_name : validate output (output_): {output_}')    
    output_['method_used'] = json.dumps(output_['method_used'])
    save_data=True
    if api:
        save_data=False
        logging.info(f'## TE info Its API fields')
    ...
    if save_data:
        ...
        output_['word_highlights']= word_highlights
        output_['highlight'] = [word_highlights, key_highlight]
        logging.info(f" ### TE info template_name got while saving is {template_name}")
        if template_name and output_['method_used']:
            data_to_store={'case_id':case_id,'template_name':template_name,'extracted_data':output_}
#         logging.info(f" data to store for api is  {api} is {data_to_store}")
            template_db.insert_dict(data_to_store,'extracted_data_retrain')
        else:
            pass
            logging.info(f" ### template name and output_ is not present for this template")
    ...
    logging.info(f'## TE info Updating ocr in ocr_table')
...
def extraction():
    data = request.json
    logging.info(f'## TE info func_name : extraction_main \n input (data) : {data}')
...
def extraction_main(data,api=False):
    ...
    logging.info(f'## TE info request data: {data}')
    if data is None:
        return {'flag': False}
    ...
    with zipkin_span(
                ...
                if documnet_splitter or documnet_splitter == "true":
                    ...
                    for values in json.loads(single_template_data):
                        ...
                        if highlight:
                            ...
                            extraction_db.execute(query, params=params)
                        extract_end_time=tt()
                        logging.info(f" Time Consumed for entire Extraction for template {single_template} route is {str(extract_end_time-start_extrct_time)}")

                else:
                    logging.info(f'## TE info entering else condition with documnet_splitter : {documnet_splitter}')
        ...
        logging.info(f"## TE info func_name : extract_main \n final_output response_data is  : {response_data} \n api is {api}")
...
def check_extraction_api():
    ...
    try:
        logging.info("checking extraction api status")
...
def predict_with_svm(word_list, div):
    ...
    try:
        logging.info(F"file found and file name is {vectorizer_filename},{model_filename}")
        ...
        logging.info(F"file not found and file name is {vectorizer_filename},{model_filename}")
        return {}
    try:
...
def find_table_header_non(each_value, ocr_data, header):
    ...
    """
    logging.info(f"header got is {header}")
    return_list = []
    ...
    if return_list:
        ...
        if head:
            ...
            logging.info(f"dif is {dif}")
            if dif > 40:
                logging.info(F"retuing ''")
                return ''
        else:
            return ''
    ...
    if return_list:
        ...
        for x in return_list:
            ...
            if dis <= 20 and x['left'] > each_value['left']:
                out_re.append(x)
        logging.info(f"getting near based on top {out_re}")
        if out_re:
            nearest_dict = min(out_re, key=lambda x: abs(x['left'] - each_value['left']))
            logging.info(f"nearest_dict is {nearest_dict} and each_value is {each_value}")
            if nearest_dict:
                return nearest_dict  
            else:
                return ''
...
def find_right_above_headers(each_value, ocr_data, keys_list, date):
    ...
    Returns:
        str: The header word if found, otherwise an empty string.
    """
    logging.info(keys_list, each_value)
    return_list = []
    centroid = (each_value['left'] + each_value['right']) / 2
    head = []
    ...
    logging.info(f"return_list in find_right_above_headers is {return_list}")

    # Find the nearest header above the value
    if return_list:
        nearest_dict = min(return_list, key=lambda x: abs(x['top'] - each_value['top']))
        logging.info(nearest_dict)
...
def get_stock(page_stock, ocr_data, sto, stock_table_dicts, header, page_dict, level_2_keys_list,value_ke,gen_ke,Sec_head):
    ...
    logging.info(f"  ################ stock_last_key is {stock_last_key}")
    ...
    for word in ocr_data:
        for val_ke in value_ke:
            if val_ke in word['word'].lower() and word['top'] > stock_last_key['top']:
                return_list.append(word)
            break
    logging.info(f"   ################  return_list in get_stock is {return_list}")
    ...
    if return_list:
        ...
        logging.info(f"  ################ numerical_values in get_stock line {numerical_values} \n")
    ...
    for each in numerical_values_temp:
        ...
        if alphanumeric_value.isdigit():
            if api:
                ...
                # logging.info(f"Numerical value that is appending: {each['word']} and its length is {len(each['word'])}")
                numerical_values.append(each)
    
    # logging.info(f"Numerical values obtained: {numerical_values}")
...
def extract_non_value_from_num(numerical_values,ocr_data,header,data,page_dict,level_2_keys_list,predict,Sec_head):
    ...
    if len(numerical_values)==1: 
        ...
        dict_temp[data['word']][numerical_values[0]['word']] = numerical_values[0]['word']
        dict_wor[data['word']][numerical_values[0]['word']] = numerical_values[0]
        col_row_dict[data['word']][numerical_values[0]['word']] = [data, numerical_values[0]]
        ...
        logging.info(f" ################ second_head got is {second_head}")
        ...
        if second_head:
            ...
            logging.info(f" ################ numerical_values got is {numerical_values}")
            ...
            for line in ocr_data_line:
                ...
                if line[0]['top'] > second_head['top']:
                    logging.info(f" ################ words_list is {words_list} for this line")
                    if predict != 'DATE':
                        for word in words_list:
                            if word in page_dict and word not in level_2_keys_list:
                                logging.info(f" ################ breaking point 1")
                                temp=1
                                break
                            ...
                            if word==wo['word']:
                                for word_ in line:
                                    if 'total' in word_['word'].lower():
                                        logging.info(f" ################ total is present in {word_['word']}")
        ...
        #once we came across total keys then we will consider the value as the required value
            ...
            logging.info(F" ################ entered else sc")
            ocr_data_line=line_wise_ocr_data(ocr_data)
            total_count=0
            for line in ocr_data_line:
                temp=0
                ...
                if line[0]['top'] > data['top']:
                    ...
                    # if we didnt find any header in the next line then proced with extraction from next line
                        ...
                        logging.info(f" ################ numerical_values are {numerical_values}")
                        numerical_values = sorted(numerical_values, key=lambda x: x["right"])
                        ...
                        if numerical_values:
                            for value in numerical_values:
                                ...
                                logging.info(f" ################ head is {head_} for value is {value} ")
                                if head_:
                                    head=head_['word']
                                    if data['word'] not in dict_temp:
                                        dict_temp[data['word']]={}
    ...
    logging.info(f" ################ predicted for the key {dict_temp} and {dict_wor}")
...
def find_non_standard_values(keys_list,ocr_data_all,seg,header_all,predicted,page_dict,predict_page,level_2_keys_list,dates,high_dict,date_range_extract,general_key_varaibles,Sec_head):
    ...
    for key,predict in keys_list.items():
            ...
            logging.info(f" ################ ")
            logging.info(f" ################ ")
            logging.info(F" ################ prediction starting for the key : {key}  and predict is {predict}\n")
            logging.info(f" ################ ")
            logging.info(f" ################ ")
            logging.info(f" ################ ")
            ...
            for data in ocr_data:
                ...
                if key.lower()==word.lower():
                    ...
                    logging.info(f" ################ right next values got is {next_values}")
                    ...
                    logging.info(f" ################ numerical_values after filtering are {numerical_values}")

                    #once values are found then send them to this function to find the acuarate values
                    logging.info(f" ################ entering into extract_non_value_from_num")
                    ...
                    logging.info(f"  ################ output got from extract_non_value_from_num is {dict_temp}  and {dict_wor}")
                    logging.info(f"  ################ col_row_dict is {col_row_dict}")
                        ...
                        if predict == 'DATE':
                            for key__,head__ in dict_wor.items():
                                for ke,val in head__.items():
                                    ...
                                    logging.info(f"  ################ head from find_right_above_headers is {head}")
                                    if head:
                                        he=keys_list[head]
                                        logging.info(f"  ################ label for head is {he}")
                                        d_t=False
                                        for date_head in date_range_extract:
                                            if date_head in he.lower():
                                                rang=get_col_range(key__)
                                                logging.info(f"  ################ rang for head is {rang}")
        ...
        if gen_ke not in predicted:
            ...
            if page_stock:
                for ke,valu in stock_table_page.items():
                    if valu == page_stock:
                        sto.append(ke)
                logging.info(f"  ################ page_stock is {page_stock} and {sto}")
                logging.info(f"  ################ calling get_stock for common values extract")
                ...
                logging.info(f"  ################ output got from get_stock is {dict_temp}  and {dict_wor}")
                logging.info(f"  ################ col_row_dict is {col_row_dict}")
                if out_stock:
                    if gen_ke not in predicted:
                        predicted[gen_ke]=[]
...
def generate_tab_view(data):
    ...
    # Get existing tab_view rows (if any)
    old_tab = data.get("tab_view", {}).get("rowData", [])
    old_map = {row["fieldName"]: row for row in old_tab}
    print(f"the oldmapi is::{old_map}")
    print(f"the old_tab is::{old_tab}")
    processed = set()

    # Loop current data keys
    for field, value in data.items():
        if field == "tab_view":
            continue
        ...
        # Extract value
        if isinstance(value, dict) and "a.v" in value:
            print("her for isinstajce")
            value = list(value["a.v"].values())[0] if value["a.v"] else ""
        elif not isinstance(value, str):
            print("fot he elif here")
            value = str(value) if value else ""
    ...
    # Add old fields not in current data (preserving their metadata)
    for old_field, old_row in old_map.items():
        if old_field not in processed:
            print("UIUIUI")
            tabular_data["rowData"].append(old_row)
        print(f"the  processed are::{processed}")
    return tabular_data

def form_final_fields(data):
    print(f"the data is:::::::{data}")
    print(type(data))
    if isinstance(data, str):
        try:
            ...
            data = {}
    print(f"the data after fltering:::::::{data}")
    print(type(data))        
    extracted_field = {}
    unused_field = {}
    ...
    try:
        # ----------------------------------------------------
        # STEP 1: PROCESS TOP-LEVEL FIELDS (NO DATA LOSS)
        # ----------------------------------------------------
        for key, value in data.items():
            if key == "tab_view":
                continue  # tab_view is handled separately

            # Case 1: simple non-empty string → extracted
            if isinstance(value, str) and value.strip() != "":
                extracted_field[key] = f"E:{value}"

            # Case 2: empty string → unused
            elif isinstance(value, str) and value.strip() == "":
                unused_field[key] = value

            # Case 3: dict object → keep unused (NO LOSS)
            elif isinstance(value, dict):
                unused_field[key] = value

            # Case 4: unexpected values → keep unused
            else:
                unused_field[key] = value

        # ----------------------------------------------------
        #  STEP 2: PROCESS tab_view.rowData SAFELY
        # ----------------------------------------------------
        tab_view = data.get("tab_view", {})
        row_data = tab_view.get("rowData", []) if isinstance(tab_view, dict) else []

        normalized_rows = []

        # Normalize rows (avoid ANY .get() crash)
        for row in row_data:

            # Correct dict → keep as is
            ...
            # Any other → convert to safe dict
            else:
                normalized_rows.append({
                    "fieldName": str(row),
                    "value": ""
        ...
        # ----------------------------------------------------
        #  STEP 3: PARSE NORMALIZED ROWS
        # ----------------------------------------------------
        for row in normalized_rows:

            # After normalization row is ALWAYS dict → safe
            key = row.get("fieldName", "")
            value = row.get("value", "")

            # Case 1: simple non-empty → extracted
            if isinstance(value, str) and value.strip() != "":
                extracted_field[key] = f"E:{value}"
        ...
        print(f"## Exception occurred while parsing fields ... {e}")
    print(f"the extraced feilds is::{extracted_field}")
    print(f"the unused_field is::{unused_field}")
    return extracted_field, unused_field



#commednte night 09_12_2025
# def form_final_fields(data):
#     print(f"the data is:::::::{data}")
#     extracted_field={}
#     unused_field={}
...
def calculate_ext_percentage(final_fields,unused_fields,tenant_id,case_id):
    print(f"the final fields is:::::{final_fields}")

    print(f"the unused_fields is:::::{unused_fields}")
    ignored_fields = [
                "Total utilisations",
                "DP As Per Lead Bank",
                "HDFC BANK_DP Allocated"
        ]
    final_fields = {
    key: value for key, value in final_fields.items()
        if key not in ignored_fields
    }
    ## Code to store the extracted fields into field accuracy table
    # number_of_fields=int(os.environ['EXTRACTION_FIELD_CNT'])
    db_config['tenant_id']=tenant_id
    db = DB('queues', **db_config)
    extraction=len(final_fields)
    not_used_fields=len(unused_fields)
    #consider the no_of_fields count as the extracted fields count
    number_of_fields=len(final_fields)
    percentage=round((extraction/number_of_fields) * 100,2)
    manual=0
    manual_percentage = 0
    query = f"select `fields_extracted` from `field_accuracy` where case_id ='{case_id}'"
    query_data = db.execute_(query)
    final_fields = {
        key: value for key, value in final_fields.items()
        if key not in ignored_fields
    }

    #  Remove extracted fields from unused fields
    unused_fields = {
        key: value for key, value in unused_fields.items()
        if key not in final_fields
    }
    query_check = f"SELECT fields_edited FROM field_accuracy WHERE case_id = '{case_id}'"
    qry_data = db.execute_(query_check)

    edited_fields = {}

    if not qry_data.empty and qry_data.iloc[0].get('fields_edited'):
        try:
            edited_fields = json.loads(qry_data.iloc[0]['fields_edited'])
        except Exception:
            edited_fields = {}
    edited_fields = {
        key: value
        for key, value in edited_fields.items()
        if key not in final_fields and key not in unused_fields
    }
    if not edited_fields:
        edited_fields_clob = "NULL"
    else:
        edited_fields_json = json.dumps(edited_fields).replace("'", "''")
        edited_chunks = [
            edited_fields_json[i:i+3900]
            for i in range(0, len(edited_fields_json), 3900)
        ]
        edited_fields_clob = " || ".join(
            [f"TO_CLOB('{c}')" for c in edited_chunks]
        )
    fields_extracted_json = json.dumps(final_fields).replace("'", "''")
    print(f"the final extracted jsonis:{fields_extracted_json}")
    not_extracted_json = json.dumps(unused_fields).replace("'", "''")
    print(f"the not_extracted_json is:{not_extracted_json}")


    # Chunk function inline
    fields_extracted_chunks = [fields_extracted_json[i:i+3900] for i in range(0, len(fields_extracted_json), 3900)]
    not_extracted_chunks = [not_extracted_json[i:i+3900] for i in range(0, len(not_extracted_json), 3900)]

    fields_extracted_clob = " || ".join([f"TO_CLOB('{c}')" for c in fields_extracted_chunks])
    not_extracted_clob = " || ".join([f"TO_CLOB('{c}')" for c in not_extracted_chunks])
    ...
    if not query_data.empty:
        ...
        SET 
            fields_extracted = {fields_extracted_clob},
            fields_edited = {edited_fields_clob},
            number_of_fields = {number_of_fields},
            extraction = {extraction},
            not_used_fields = {not_used_fields},
            extraction_percentage = {percentage},
            not_extracted_fields = {not_extracted_clob},
        ...
        # INSERT new row
        query = f"""
        INSERT INTO field_accuracy
            (case_id, fields_extracted, number_of_fields, extraction, not_used_fields,
            extraction_percentage, not_extracted_fields, manual, manual_percentage)
        VALUES
            ('{case_id}', {fields_extracted_clob}, {number_of_fields}, {extraction},
            {not_used_fields}, {percentage}, {not_extracted_clob}, {manual}, {manual_percentage})
        """
        db.execute_(query)
    ...
    # if not query_data.empty:
    #     query = f"update `field_accuracy` set `fields_extracted`='{json.dumps(final_fields)}',`number_of_fields`='{number_of_fields}' ,`extraction`='{extraction}',`not_used_fields`='{not_used_fields}', `extraction_percentage`='{percentage}',`not_extracted_fields`='{json.dumps(unused_fields)}', `MANUAL_PERCENTAGE` = '{manual_percentage}', `MANUAL` = '{manual}' where case_id ='{case_id}'"
    #     db.execute_(query)
    # else:
    #     query = f"INSERT INTO `field_accuracy` ( `case_id`,`fields_extracted`,`number_of_fields`,`extraction`,`not_used_fields`, `extraction_percentage`,`not_extracted_fields` , `MANUAL`, `MANUAL_PERCENTAGE`) VALUES ('{case_id}','{json.dumps(final_fields)}','{number_of_fields}','{extraction}','{not_used_fields}','{percentage}','{json.dumps(unused_fields)}', '{manual}', '{manual_percentage}')"
    #     db.execute_(query)
    # return True

def get_mapped_key(component):
    print(f"got comp is:::{component}")
    mapping = {"Raw Materials":["RAW MATERIALS INSURED","RAW MATERIALS-IMP INSURED","RAW MATERIALS - IND. INSURED","RAW MATERIAL (LOCAL) INSURED","RAW MATERIAL (EXPORTS) INSURED"],"Finished Goods":["FINISHED GOODS INSURED","FINISH GOODS - EXPORTS INSURED","FINISHED GOODS (BLUK DRUG) INSURED","FINISHED GOODS ( FORMULATION) INSURED","FINISHED GOODS(EXPORT/FORMULATION  B.D)INSURED"],"Total Stock":["TOTAL STOCKS INSURED","TOTAL STOCK (PC) INSURED"],"Work in Process":["WORK IN PROGRESS INSURED","Semi Finished Goods","STOCK IN PROCESS INSURED","WORK IN PROGRESS (LOCAL) INSURED","WORK IN PROGRESS (EXPORTS) INSURED"],"Stores and Spares":["STORES & SPARES INSURED","STOCK & STORES INSURED"],"Stock in Transit":["STOCK IN TRANSIT"],"goods in transist":["GOODS IN TRANSIT INSURED"],"Domestic Stock":["DOMESTIC STOCKS INSURED","Domestic Stock"],"Export Stock":["EXPORT STOCKS INSURED"],"Sales":["SALES"],"Debtors <30 days":["Debtors <30 days","BOOK DEBTS UPTO 30 DAYS"],"Debtors <60 days":["Debtors <60 days","BOOK DEBTS UPTO 60 DAYS"],"Debtors <90 days":["Debtors <90 days","BOOK DEBTS UPTO 90 DAYS"],"Debtors <120 days":["Debtors <120 days","BOOK DEBTS UPTO 120 DAYS"],"Debtors <150 days":["Debtors <150 days","BOOK DEBTS UPTO 150 DAYS"],"Debtors <180 days":["Debtors <180 days","BOOK DEBTS UPTO 180 DAYS","EXPORT DEBTORS<180 DAYS INSURED"],"Debtors - Exports":["BOOK DEBTS -EXPORTS"],"Debtors - Domestic":["BOOK DEBTS -DOMESTIC"],"Debtiors of Group Companies":["DEBTORS OF GROUP COMPANIES"],"Receivables":["RECEIVABLES"],"Domestic receivables":["DOMESTIC RECEIVABLES"],"Export receivables":["EXPORT RECEIVABLES"],"Total Creditors":["CREDITORS","Trade Creditors"],"Creditors PC":["CREDITORS (PC)"],"Unpaid Stocks":["UNPAID STOCKS"],"DALC":["LESS : DALC"],"Advances paid to suppliers":["ADD : ADVANCE TO SUPPLIER"],"Debtors PC":["DEBTORS (PC)"],"Advances from Debtors/Customers":["LESS: ADVANCE RECIEVED"],"RAW MATERIALS INSURED":["RAW MATERIALS INSURED"],"STOCK IN PROCESS INSURED":["STOCK IN PROCESS INSURED"],"FINISHED GOODS INSURED":["FINISHED GOODS INSURED"],"STORES & SPARES INSURED":["STORES & SPARES INSURED"],"EXPORT STOCKS INSURED":["EXPORT STOCKS INSURED"],"RAW MATERIALS-IMP INSURED":["RAW MATERIALS-IMP INSURED"],"RAW MATERIALS - IND. INSURED":["RAW MATERIALS - IND. INSURED"],"WORK IN PROGRESS INSURED":["WORK IN PROGRESS INSURED"],"PACKING MATERIAL INSURED":["PACKING MATERIAL INSURED"],"GOODS IN TRANSIT INSURED":["GOODS IN TRANSIT INSURED"],"TOTAL STOCKS INSURED":["TOTAL STOCKS INSURED"],"FINISH GOODS - EXPORTS INSURED":["FINISH GOODS - EXPORTS INSURED"],"TOTAL STOCK (PC) INSURED":["TOTAL STOCK (PC) INSURED"],"CONSUMABLES INSURED":["CONSUMABLES INSURED"],"CONSUMABLE SPARES INSURED":["CONSUMABLE SPARES INSURED"],"TELECOM DEVISION INSURED":["TELECOM DEVISION INSURED"],"LOCAL STOCKS INSURED":["LOCAL STOCKS INSURED"],"COMPONENTS INSURED":["COMPONENTS INSURED"],"STOCK & STORES INSURED":["STOCK & STORES INSURED"],"OTHER RM INSURED":["OTHER RM INSURED"],"OILS INSURED":["OILS INSURED"],"FG INSURED":["FG INSURED"],"STOCK AT PORT INSURED":["STOCK AT PORT INSURED"],"STOCK AT ALD INSURED":["STOCK AT ALD INSURED"],"Semi Finished Goods":["Semi Finished Goods"],"STOCK (> 180 DAYS) INSURED":["STOCK (> 180 DAYS) INSURED"],"STOCK WITHOUT AGEING INSURED":["STOCK WITHOUT AGEING INSURED"],"STOCK < 90 DAY INSURED":["STOCK < 90 DAY INSURED"],"STOCK < 120 DAY INSURED":["STOCK < 120 DAY INSURED"],"STOCK < 270 DAY INSURED":["STOCK < 270 DAY INSURED"],"STOCK < 30 DAY INSURED":["STOCK < 30 DAY INSURED"],"STOCK < 60 DAY INSURED":["STOCK < 60 DAY INSURED"],"STOCK <180 DAY INSURED":["STOCK <180 DAY INSURED"],"ADD : ADVANCE TO SUPPLIER":["ADD : ADVANCE TO SUPPLIER"],"STOCKS UPTO 365 DAYS INSURED":["STOCKS UPTO 365 DAYS INSURED"],"RAW MATERIAL (LOCAL) INSURED":["RAW MATERIAL (LOCAL) INSURED"],"RAW MATERIAL (EXPORTS) INSURED":["RAW MATERIAL (EXPORTS) INSURED"],"WORK IN PROGRESS (LOCAL) INSURED":["WORK IN PROGRESS (LOCAL) INSURED"],"WORK IN PROGRESS (EXPORTS) INSURED":["WORK IN PROGRESS (EXPORTS) INSURED"],"FINISHED GOODS (BLUK DRUG) INSURED":["FINISHED GOODS (BLUK DRUG) INSURED"],"FINISHED GOODS ( FORMULATION) INSURED":["FINISHED GOODS ( FORMULATION) INSURED"],"PACKING MATERIAL (EXPORTS) INSURED":["PACKING MATERIAL (EXPORTS) INSURED"],"PACKING MATERIAL (LOCAL) INSURED":["PACKING MATERIAL (LOCAL) INSURED"],"FINISHED GOODS(EXPORT/FORMULATION  B.D)INSURED":["FINISHED GOODS(EXPORT/FORMULATION  B.D)INSURED"],"STOCK <150 DAY INSURED":["STOCK <150 DAY INSURED"],"stock special":["stock special"],"stock special35":["stock special35"],"BOOK DEBTS UPTO 75 DAYS":["BOOK DEBTS UPTO 75 DAYS"],"BOOK DEBTS":["BOOK DEBTS"],"BOOK DEBTS -EXPORTS":["BOOK DEBTS -EXPORTS"],"DEBTORS":["DEBTORS"],"BLUE CHIP DEBTORS":["BLUE CHIP DEBTORS"],"DEBTIORS (PC)":["DEBTIORS (PC)"],"EXPORT RECEIVABLES":["EXPORT RECEIVABLES"],"RECEIVABLES":["RECEIVABLES"],"DOMESTIC RECEIVABLES":["DOMESTIC RECEIVABLES"],"LOCAL BOOK DEBTS":["LOCAL BOOK DEBTS"],"BOOK DEBTS UPTO 90 DAYS":["BOOK DEBTS UPTO 90 DAYS"],"BOOK DEBTS UPTO 120 DAYS":["BOOK DEBTS UPTO 120 DAYS"],"BOOK DEBTS UPTO 180 DAYS":["BOOK DEBTS UPTO 180 DAYS"],"BOOK DEBTS UPTO 60 DAYS":["BOOK DEBTS UPTO 60 DAYS"],"EXPORT DRS - 90 DAYS":["EXPORT DRS - 90 DAYS"],"BOOK DEBTS UP TO 7 DAYS":["BOOK DEBTS UP TO 7 DAYS"],"NOT APPLICABLE FOR DP-DEBTORS UPTO 365 DAYS-BASEL":["NOT APPLICABLE FOR DP-DEBTORS UPTO 365 DAYS-BASEL"],"BOOK DEBTS UPTO 30 DAYS":["BOOK DEBTS UPTO 30 DAYS"],"DEBTORS OF GROUP COMPANIES":["DEBTORS OF GROUP COMPANIES"],"DEBTORS WITHOUT AGEING":["DEBTORS WITHOUT AGEING"],"LOANS & ADVANCES":["LOANS & ADVANCES"],"BOOK DEBTS UPTO 140 DAYS":["BOOK DEBTS UPTO 140 DAYS"],"BOOK DEBTS UPTO 365 DAYS":["BOOK DEBTS UPTO 365 DAYS"],"EXPORT DEBTORS<180 DAYS INSURED":["EXPORT DEBTORS<180 DAYS INSURED"],"DOMESTIC DEBTORS<120 DAYS INSURED":["DOMESTIC DEBTORS<120 DAYS INSURED"],"BOOK DEBTS UPTO 150 DAYS":["BOOK DEBTS UPTO 150 DAYS"],"BOOK DEBTS UPTO 210 DAYS":["BOOK DEBTS UPTO 210 DAYS"],"BOOK DEBTS UPTO 240 DAYS":["BOOK DEBTS UPTO 240 DAYS"],"BOOK DEBTS UPTO 270 DAYS":["BOOK DEBTS UPTO 270 DAYS"],"BOOK DEBTS UPTO 300 DAYS":["BOOK DEBTS UPTO 300 DAYS"],"BOOK DEBTS UPTO 45 DAYS":["BOOK DEBTS UPTO 45 DAYS"],"EXPORTS DEBTORS < 120 DAYS":["EXPORTS DEBTORS < 120 DAYS"],"BOOK DEBTS UP TO 135 DAYS":["BOOK DEBTS UP TO 135 DAYS"],"BOOK DEBTS UP TO 105 DAYS":["BOOK DEBTS UP TO 105 DAYS"],"CREDITORS":["CREDITORS"],"CREDITORS (PC)":["CREDITORS (PC)"],"UNPAID STOCKS":["UNPAID STOCKS"],"ACCEPTANCES":["ACCEPTANCES"],"LESS : DALC":["LESS : DALC"],"LESS : IMPORT DA BILLS":["LESS : IMPORT DA BILLS"],"RAW MATERIALS":["RAW MATERIALS"],"STOCK IN PROCESS":["STOCK IN PROCESS"],"FINISHED GOODS":["FINISHED GOODS"],"STORES & SPARES":["STORES & SPARES"],"RAW MATERIALS-IMP":["RAW MATERIALS-IMP"],"TOTAL STOCKS":["TOTAL STOCKS"],"EARMARKING OF DP":["EARMARKING OF DP"],"LESS: ADVANCE RECIEVED":["LESS: ADVANCE RECIEVED"]}
    for master, variations in mapping.items():
        if component in variations:
            return master
    print(f"the comp is::::{component}")
    return component



def ensure_all_components_in_ocr(case_id, PARTY_ID, extraction_db):
    """
    Ensures all active components from AGE_MARGIN_WORKING_UAT are present 
    in their respective OCR category columns
    """
    
    # Mapping of component_category to OCR column names
    CATEGORY_TO_OCR_COLUMN = {
        'Stocks': 'STOCKS',
        'Stock': 'STOCKS',
        'Creditors': 'CREDITORS',
        'Debtors': 'DEBTORS',
        ...
        # Add more mappings as needed based on your COMPONENT_MASTER categories
    }
    
    
    # Step 1: Get all active components for this party
    query = f"""
        ...
        AND is_active = 1
    """
    age_rows = extraction_db.execute_(query).to_dict(orient='records')
    print(f"Active components for {PARTY_ID}: {age_rows}")
    
    if not age_rows:
        print(f"No active components found for party {PARTY_ID}")
        return
    
    # Step 2: Build a mapping of category -> list of components
    category_components = {}
    ...
    for row in age_rows:
        ...
        # Get component category from COMPONENT_MASTER
        comp_query = f"""
            SELECT CAST(component_category AS VARCHAR2(4000)) as component_category
            FROM COMPONENT_MASTER
            WHERE CAST(component_name AS VARCHAR2(4000)) = '{component_name}'
            AND CAST(status AS VARCHAR2(4000)) = 'ACTIVE'
        """
        comp_res = extraction_db.execute_(comp_query).to_dict(orient='records')
        print(f"the comp pres is::::::{comp_res}")
        if comp_res:
            category = comp_res[0].get('component_category')
            if category:
                if category not in category_components:
                    category_components[category] = []
                category_components[category].append(component_name)
    
    print(f"Category components mapping: {category_components}")
    
    # Step 3: For each category, update the corresponding OCR column
    for category, components in category_components.items():
        ocr_column = CATEGORY_TO_OCR_COLUMN.get(category)
        print(f"the ocr col is::{ocr_column}")
        
        if not ocr_column:
            print(f"Warning: No OCR column mapping for category '{category}'")
            continue
        
        # Fetch current OCR data for this column
        fetch_query = f"""
            SELECT {ocr_column}
            FROM ocr
            WHERE case_id = '{case_id}'
        """
        ocr_data_df = extraction_db.execute_(fetch_query)
        
        if ocr_data_df.empty:
            print(f"No data found for case_id {case_id}")
            continue
        
        ocr_json_str = ocr_data_df.iloc[0][ocr_column]
        
        if not ocr_json_str or ocr_json_str == 'null':
            ocr_data = {}
        else:
            try:
                ocr_data = json.loads(ocr_json_str)
                print(f"the ocr data is:::::::::::::::{ocr_data}")
            except:
                print(f"Error parsing JSON for {ocr_column}")
                ocr_data = {}
        original_ocr_data = ocr_data.copy()
        print(f"Processing components: {components}")
        # Step 4: Check if each component exists, if not add with empty value
        modified = False
        # print(f"the compoents are::::{components}")
        for component in components:
            new_key = get_mapped_key(component)
            print(f"Component: '{component}' -> Mapped key: '{new_key}'")
             #  protect existing filled values
            # if component in ocr_data and ocr_data[component]:
            #     print(f"Component '{component}' already has value: {ocr_data[component][:50]}...")
            #     ###removing the new key from the ocr bcz of compoenent is already exit and it has the value
                
            #     continue

            if new_key in ocr_data and ocr_data[new_key]:
                print(f"Copying value from '{new_key}' to '{component}'")
                ocr_data[component] = ocr_data.pop(new_key)
                print(f"the ocr data iss:::::{ocr_data}")
            # if new_key in ocr_data:
            #     ocr_data[component] = ocr_data.pop(new_key)
            elif component not in ocr_data:
                print(f"Adding missing component '{component}' with empty value")
                ocr_data[component] = ''
            else:
                print(f"Component '{component}' exists but is empty")
                print(f"unknwo scenores component is {component} and new key is::::{new_key}")   
            if new_key in ocr_data and component in ocr_data and new_key != component:
            # if new_key in ocr_data and component in ocr_data and new_key not in components:
                print(f"Removing duplicate key '{new_key}'")
                ocr_data.pop(new_key)
            # if "tab_view" in ocr_data and "rowData" in ocr_data["tab_view"]:
            #     ocr_data["tab_view"]["rowData"] = [
            #         row for row in ocr_data["tab_view"]["rowData"]
            #         if row.get("fieldName", "").strip().lower() != new_key.strip().lower()
            #     ]
            # Delete tab_view row ONLY when mapping alias → master
            print(f"the ocr data Delete tab_view row ONLY when mapping alias → master is:::{ocr_data}")
            if (
                "tab_view" in ocr_data
                and "rowData" in ocr_data["tab_view"]
                and new_key != component
            ):
                ocr_data["tab_view"]["rowData"] = [
                    row for row in ocr_data["tab_view"]["rowData"]
                    if row.get("fieldName", "").strip().lower() != new_key.strip().lower()
                ]
            print(f"the ocr data is::Delete tab_view row ONLY when mapping alias → master{ocr_data}")
            modified = True

        if modified:
            print(f"Before regeneration - ocr_data keys: {list(ocr_data.keys())}")
            # ---- ADD THIS BLOCK (START) ----
            # All canonical (mapped) master keys for current components
            mapped_master_keys = {get_mapped_key(c) for c in components}
            print(f"the mapped masterk ey are ::{mapped_master_keys}")
            # ---- ADD THIS BLOCK (END) ----
            print(f"the ocr data is:::at before for loop preseve{ocr_data}")
            print(f"the original_ocr_data is:::at before for loop preseve{original_ocr_data}")
            #  Preserve all fields from original that aren't in the component list
            for key, value in original_ocr_data.items():
                if (
                    key not in ['tab_view']
                    and key not in ocr_data
                    and not any(
                    key == get_mapped_key(c) and c in ocr_data
                    for c in components
                )
                ):
                # if key not in ['tab_view'] and key not in ocr_data:
                    # This field was in original but not in current - preserve it
                    print(f"Preserving original field: '{key}' = '{str(value)[:50]}...'")
                    ocr_data[key] = value
            
            print(f"After preservation - ocr_data keys: {list(ocr_data.keys())}")
            print(f"the ocr data is:::at after preseve{ocr_data}")
            ocr_data['tab_view'] = generate_tab_view(ocr_data)
            
            # Update the database with chunked CLOB
            value_json = json.dumps(ocr_data)
            chunk_size = 4000
            chunks = [value_json[i:i + chunk_size] for i in range(0, len(value_json), chunk_size)]
            
            sql = f"UPDATE ocr SET {ocr_column} = "
            for chunk in chunks:
                sql += "TO_CLOB('" + chunk.replace("'", "''") + "') || "
            sql = sql[:-4] + f" WHERE case_id='{case_id}'"
            
            extraction_db.execute_(sql)
            print(f"Updated {ocr_column} for case_id {case_id}")
            print(f"Final field count: {len([k for k in ocr_data.keys() if k != 'tab_view'])}")

def update_db_ntable(predicted_fields, case_id, tenant_id,mapping_dict={}, seg='non_stand'):
    """
    Update database tables with predicted fields for a given case_id and tenant_id.
    Fully based on party_id instead of customer_name.
    """
    import logging
    import json
    from concurrent.futures import ThreadPoolExecutor

    logging.info(f"Update into DB fields got: {predicted_fields}")
    print(f"Update into DB fields got: {predicted_fields}")

    print(f"the mapping dict is::::::::::{mapping_dict}")
    db_config['tenant_id'] = tenant_id
    extraction_db = DB('extraction', **db_config)
    template_db = DB('template_db', **db_config)

    executor = ThreadPoolExecutor(max_workers=2)  # For async tasks

    # Read corpus from JSON
    json_file_path = f"/var/www/extraction_api/app/extraction_folder/non_stand/non_stand_db.json"
    corpus = read_corpus_from_json(json_file_path)[0]

    # Required fields for the segment
    ...
    for table in req_fields:
        ...
        party_id_rows = extraction_db.execute_(qry).to_dict(orient='records')

        print(f"the partyid is::::::::{party_id_rows}")
        # Extract clean value
        party_id = party_id_rows[0].get("party_id") or party_id_rows[0].get("PARTY_ID")
        print(f"Extracted party_id: {party_id}")
        ...
        WHERE party_id='{party_id}'
        """
        trained_data = template_db.execute_(trained_query).to_dict(orient="records")
        print(f"the trined data is::{trained_data}")
        print("hiiii")
        if trained_data and trained_data[0].get("new_add_fields"):
            print("hellll")
            try:
                trained_json = json.loads(trained_data[0]["new_add_fields"])
                print(f"the traine json is:::::::::{trained_json}")
                # Loop: stocks, debtors, advances...
                for tbl, val in trained_json.items():

                    # Extract list: new_add_fields = [...]
                    new_fields_list = val.get("new_add_fields", [])

                    if not new_fields_list:
                        continue

                    # Convert "stocks" → "Stocks"
                    table_div = tbl.replace("_", " ").title().replace(" ", "_")

                    # Add each new field into corpus
                    for new_field in new_fields_list:
                        corpus[new_field] = table_div
                        print(f"[NEW FIELD MAPPED] {new_field} → {table_div}")

            except Exception as e:
                logging.error(f"Error parsing trained_info new_fields JSON: {e}")

    # --------------------------------------------------------------------
    # Now corpus contains:
    # - Standard fields from required_fields_table
    # - Dynamic new_add_fields from trained_info
    # --------------------------------------------------------------------
    ...
    db_dict = {}
    print(f"Corpus: {corpus}")
    print(f"the party id is::{party_id}")
    # db_dict = {}
    if party_id:
        print("_____#@")
        query = f"""
        ...
        """
        age_rows = extraction_db.execute_(query).to_dict(orient='records')
        print(f"the age rows ae:::::::::::{age_rows}")
        for row in age_rows:
            component_name = row.get("component_name")
            if not component_name:
                continue
            ...
            """
            comp_res = extraction_db.execute_(comp_query).to_dict(orient='records')
            print(f"the comp res is:::::::{comp_res}")
            if not comp_res:
                continue
            component_category = comp_res[0].get("component_category", "").strip()
            print(f"the comp categery is::::::{component_category}")
            #print(f"tab_map is....:{tab_map}")

            if not component_category:
                continue
            tab = component_category.lower()
            field_name = component_name
            if tab not in db_dict:
                db_dict[tab] = {}
            db_dict[tab].setdefault(field_name, "")
        print(f"her after the component name:::::::::::{db_dict}")
        TAB_CATEGORY_MAP = {"stock":"Stocks","STOCK":"Stocks","INVENTORY":"Inventory","DEBTORS":"Debtors","RECEIVABLES":"Receivable","RECEIVABLE":"Receivable","CREDITORS":"Creditors","ADVANCES":"Advances"}
        print("Before correction:", db_dict)

        #  PLACE THIS HERE
        db_dict = {
            TAB_CATEGORY_MAP.get(k, k): v
            for k, v in db_dict.items()
        }

        print("After correction:", db_dict)

    # Organize predicted fields into db_dict per table
    # Organize predicted fields into db_dict per table
    for key, value in predicted_fields.items():
        table = None
        #  First priority: already decided by component logic
        for tab, fields in db_dict.items():
            if key in fields:
                print("Hii")
                table = tab
                break
        # Second priority: corpus mapping
        if not table and key in corpus:
            print("HELL")
            table = corpus[key]
        # If still unknown → skip (do NOT break core flow)
        if not table:
            continue
    ...
    print(f"her after the agemaric got epty we will make logic here:::{db_dict}")
    # for key, value in predicted_fields.items():
    #     if key in corpus:
    #         table = corpus[key]
    #         if table not in db_dict:
    #             db_dict[table] = {}
    #         db_dict[table][key] = value
    # print(f"the db dict is:::::::{db_dict}")
    # Fetch party_id and customer_name from OCR
    query = f"SELECT customer_name, party_id FROM ocr WHERE case_id='{case_id}'"
    result = extraction_db.execute_(query).to_dict(orient="records")
    if not result:
        logging.warning(f"No OCR record found for case_id {case_id}")
        return False

    PARTY_ID = result[0]['party_id']
    CUSTOMER_NAME = result[0].get('customer_name', '')

    logging.info(f"Fetched PARTY_ID: {PARTY_ID}, CUSTOMER_NAME: {CUSTOMER_NAME}")

    # Get unsubscribed fields based on party_id if available
    unsubscribed_fields = []
    try:
        if PARTY_ID:
            query = f"SELECT UNSUBCRIBED_FIELDS FROM trained_info WHERE party_id='{PARTY_ID}'"
            unsub_fields_db = template_db.execute_(query)['UNSUBCRIBED_FIELDS'].to_list()
            if unsub_fields_db:
                unsubscribed_fields = json.loads(unsub_fields_db[0]) if unsub_fields_db[0] else []

                # unsubscribed_fields = json.loads(unsub_fields_db[0])
            print(f"Unsubscribed fields: {unsubscribed_fields}")
    except Exception as e:
        logging.warning(f"Error fetching unsubscribed fields: {e}")

    # # Dynamic fields (component + duplicated)
    # dynamic_fields = {}
    # if PARTY_ID:
    #     query = f"SELECT component_fields, duplicated_fields FROM dynamic_fields WHERE party_id='{PARTY_ID}'"
    #     dynamic_result = extraction_db.execute_(query).to_dict(orient='records')

    #     if dynamic_result:
    #         record = dynamic_result[0]
    #         try:
    #             component_fields = json.loads(record.get('component_fields', '{}'))
    #         except:
    #             component_fields = {}
    #         try:
    #             duplicated_fields = json.loads(record.get('duplicated_fields', '{}'))
    #         except:
    #             duplicated_fields = {}

    #         # Merge component_fields and duplicated_fields
    #         merged_dynamic_fields = {}
    #         for tab, fields in component_fields.items():
    #             merged_dynamic_fields[tab] = set(fields)
    #         for tab, fields in duplicated_fields.items():
    #             if tab not in merged_dynamic_fields:
    #                 merged_dynamic_fields[tab] = set()
    #             merged_dynamic_fields[tab].update(fields)
    #         # Convert sets to lists
    #         merged_dynamic_fields = {k: list(v) for k, v in merged_dynamic_fields.items()}

    #         # Add dynamic fields to db_dict with empty string if not present
    #         for tab, fields in merged_dynamic_fields.items():
    #             if tab not in db_dict:
    #                 db_dict[tab] = {}
    #             for field in fields:
    #                 db_dict[tab].setdefault(field, '')
    
    #commented 01_12_2025

    if PARTY_ID:
        query = f"SELECT new_add_fields FROM trained_info WHERE party_id='{PARTY_ID}'"
        new_fields_res = template_db.execute_(query).to_dict(orient='records')
        print(f"the new fields is::::::{new_fields_res}")
        if new_fields_res:
            try:
                new_fields_json = json.loads(new_fields_res[0].get('new_add_fields', '{}'))
            except:
                new_fields_json = {}
            # Loop over each tab in new_fields_json
            for tab, tab_data in new_fields_json.items():
                # Extract array from new_add_fields key
                fields = tab_data.get('new_add_fields', [])
                if not fields:
                    continue
                table_div = tab.replace("_", " ").title().replace(" ", "_")
                print(f"the tab vie is:::::::::{table_div}")
                # Ensure tab exists in db_dict
                if table_div not in db_dict:
                    db_dict[table_div] = {}
                
                for field_name in fields:
                    # Only set empty string if field doesn't exist
                    # Don't overwrite existing values
                    if field_name not in db_dict[table_div]:
                        db_dict[table_div][field_name] = ''
                    elif not db_dict[table_div][field_name]:  # If field exists but is empty/None/False
                        db_dict[table_div][field_name] = ''    
                # # Ensure tab exists in db_dict
                # if tab not in db_dict:
                #     db_dict[tab] = {}
                # for field_name in fields:
                #     db_dict[tab].setdefault(field_name, '')
        print(f"the db dict aftere new add fields is:::::::{db_dict}")
    

    # Process required fields and map them properly
    print(f"the req fields is::::{req_fields}")
    for div in req_fields:
        div_tab = div["table_div"].lower()
        req_fi = json.loads(div["fields"])
        print(f"the req_fi is::::{req_fi}")
        out_dic = db_dict.get(div_tab, {})
        print(f" Processing table: {div_tab}")
        print(f" Required fields structure: {req_fi}")
        print(f" Current out_dic keys: {list(out_dic.keys())}")
        for re in req_fi:
            tem_fla = False
            for fie in re:
                if fie in out_dic:
                    if re[0] in out_dic:
                        ...
                        if fie != re[0]:
                            out_dic.pop(fie)
                        tem_fla = True
            if re[0] in unsubscribed_fields:
                out_dic[re[0]] = ''
            elif re[0] not in out_dic:
                out_dic[re[0]] = ''
            
            # if not tem_fla or re[0] in unsubscribed_fields:
            #     # out_dic.pop(re[0], None)
            #     out_dic[re[0]] = ''
        db_dict[div_tab] = out_dic

    print(f"db_dict after processing required fields: {db_dict}")
    # FINAL STEP: Empty all unsubscribed fields across all tables
    if unsubscribed_fields:
        for tab in db_dict:
            for field in unsubscribed_fields:
                if field in db_dict[tab]:
                    db_dict[tab][field] = ''
        logging.info(f"Emptied unsubscribed fields: {unsubscribed_fields}")
    try:
        # Fetch column names from Oracle metadata instead of relying on table data
        ocr_columns_df = extraction_db.execute_(
            "SELECT COLUMN_NAME FROM ALL_TAB_COLUMNS WHERE TABLE_NAME = 'OCR'"
        )
        valid_columns = set(
            ocr_columns_df['COLUMN_NAME'].str.lower().tolist()
        )
        print(f"the valid colnnms are:::::::::{valid_columns}")
    except Exception as e:
        logging.exception("Error fetching OCR column names.")
        return False

    # Update OCR table with processed fields
    parsed_fields = {}
    unused_fields = {}
    print(f"the db dict is::____:::::{db_dict}")
    for key, value in db_dict.items():
        temp = key.lower()
        try:
            if temp not in valid_columns:
                logging.warning(f"Skipping '{key}' — column not found in OCR table.")
                continue
        except Exception as e:
            print(f"the error iss while skipping:::{e}")
        ...
        if column:
            print(f"the col is:::::{column}")
            column = json.loads(column)
            # --- APPLY MAPPING: rename old keys to new keys --
            for key_, value_ in value.items():
                if value_:  # Only update if value is not empty
                    column[key_] = value_
                elif key_ not in column:  # Only add empty if field doesn't exist
                    column[key_] = value_
            # for key_, value_ in value.items():
            #     column[key_] = value_
            ...
            for new_key, old_key in mapping_dict.items():
                if old_key in column:
                    column[new_key] = column.pop(old_key)
                
            value = column
            print(f"the value is::::::{value}")
        print(f"the maping aft again query:{mapping_dict}")

        
        value['tab_view'] = generate_tab_view(value)

        # Split into chunks for TO_CLOB update
        # value_json = json.dumps(value)
        # print(f"the value json is:::::::{value_json}")
        # chunk_size = 4000
        # chunks = [value_json[i:i + chunk_size] for i in range(0, len(value_json), chunk_size)]
        ...
        try:
            extracted_field, unused_field = form_final_fields(value)
            parsed_fields.update(extracted_field)
            unused_fields.update(unused_field)
        except Exception as e:
            logging.exception(f"Error parsing final fields: {e}")
    if PARTY_ID:#commednted 01_12_2025_16_57
        try:
            ensure_all_components_in_ocr(case_id, PARTY_ID, extraction_db)
            logging.info(f"Successfully ensured all components in OCR for case_id: {case_id}")
            CATEGORY_TO_OCR_COLUMN = {
            'Stocks': 'STOCKS',
            'Stock': 'STOCKS',
            'Creditors': 'CREDITORS',
            ...
            # Clear and re-populate parsed_fields and unused_fields
            parsed_fields.clear()
            unused_fields.clear()

            # Re-fetch all OCR columns
            for ocr_column in set(CATEGORY_TO_OCR_COLUMN.values()):
                refetch_query = f"""
                    SELECT {ocr_column}
                    FROM ocr
                    WHERE case_id = '{case_id}'
                """
                updated_ocr_df = extraction_db.execute_(refetch_query)
                
                if not updated_ocr_df.empty and updated_ocr_df.iloc[0][ocr_column]:
                    updated_value = updated_ocr_df.iloc[0][ocr_column]
                    
                    try:
                        extracted_field, unused_field = form_final_fields(updated_value)
                        parsed_fields.update(extracted_field)
                        print(f"Final parsed fields: {parsed_fields}")
                        unused_fields.update(unused_field)
                        print(f"unused_fields fields: {unused_fields}")
                    except Exception as e:
                        logging.exception(f"Error re-parsing {ocr_column}: {e}")


        except Exception as e:
            logging.exception(f"Error ensuring components in OCR: {e}")
    # Handle date_stat fields
    try:
        qry = f"SELECT date_stat FROM ocr WHERE case_id='{case_id}'"
        cus_date_df = extraction_db.execute_(qry)
        if not cus_date_df.empty:
            cus_date_df.columns = [col.lower() for col in cus_date_df.columns]
            for data in cus_date_df.to_dict(orient='records'):
                for k, v in data.items():
                    if v is None or v == "":
                        unused_fields[k] = ""
                    else:
                        parsed_fields[k] = f"E:{v}"
        print("enter for calc extr paerentage:::")                
        executor.submit(calculate_ext_percentage, parsed_fields, unused_fields, tenant_id, case_id)
    except Exception as e:
        logging.exception(f"Error calculating extracted percentage: {e}")

    logging.info(f"Final parsed fields: {parsed_fields}")
...
####Working function_13_10_2025_commented for another update fucntio above
# def update_db_ntable(predicted_fields, case_id, tenant_id, seg='non_stand'):
#     logging.info(F"upadte into sdb fields got are {predicted_fields}")
...
#     logging.info(f"corpus is {corpus}")
#     for key, value in predicted_fields.items():
...
#     logging.info(F"db_dict before req {db_dict}")

#     query = f"SELECT `customer_name`, `party_id` FROM `ocr` WHERE `case_id` = '{case_id}'"
#     result = extraction_db.execute_(query).to_dict(orient="records")
#     CUSTOMER_NAME = result[0]['customer_name']
#     PARTY_ID = result[0]['party_id']
#     logging.info(f"CUSTOMER_NAME got is {CUSTOMER_NAME}, PARTY_ID is {PARTY_ID}")
...
#             logging.info(f"unsubcribed_fields got is {unsubcribed_fields}")
#     except:
#         pass
    
#     # Check if we have a valid PARTY_ID
#     if PARTY_ID:
#         # Query the database to get the saved component and duplicated fields for this party_id
#         query = f"SELECT component_fields, duplicated_fields FROM dynamic_fields WHERE party_id = '{PARTY_ID}'"
#         dynamic_fields_result = extraction_db.execute_(query).to_dict(orient='records')
#         # If results were found in the database
#         if dynamic_fields_result:
#             record = dynamic_fields_result[0]
#             print(f"Dynamic fields record: {record}")
#             # Try to convert the JSON string of component_fields into a Python dictionary
#             try:
#                 component_fields = json.loads(record['component_fields']) if record['component_fields'] else {}
#             except Exception as e:
#                 component_fields = {}    # Set as empty if conversion fails
#                 logging.error(f"Error parsing component fields: {e}")

#             # Try to convert the JSON string of duplicated_fields into a Python dictionary
#             try:
#                 duplicated_fields = json.loads(record['duplicated_fields']) if record['duplicated_fields'] else {}
#             except Exception as e:
#                 duplicated_fields = {}    # Set as empty if conversion fails
#                 logging.error(f"Error parsing duplicated fields: {e}")

#             # merged_dynamic_fields = {}
#             # for tab, fields in {**component_fields, **duplicated_fields}.items():
#             #     if tab not in merged_dynamic_fields:
#             #         merged_dynamic_fields[tab] = set()
#             #     merged_dynamic_fields[tab].update(fields)

#             # merged_dynamic_fields = {k: list(v) for k, v in merged_dynamic_fields.items()}
#             # logging.info(f"Merged dynamic fields: {merged_dynamic_fields}")

#             # Initialize a dictionary to hold merged fields from both component_fields and duplicated_fields
#             merged_dynamic_fields = {}

#             # Step 1: Add all component_fields to the merged dictionary
#             for tab, fields in component_fields.items():
#                 # Use a set to avoid duplicate field names
#                 merged_dynamic_fields[tab] = set(fields)

#             # Step 2: Add all duplicated_fields to the same merged dictionary
#             for tab, fields in duplicated_fields.items():
#                 if tab not in merged_dynamic_fields:
#                     merged_dynamic_fields[tab] = set()      # Initialize if not already present
#                 merged_dynamic_fields[tab].update(fields)   # Merge in additional fields

#             # Convert sets back to lists since sets are not JSON serializable and might be harder to work with later
#             merged_dynamic_fields = {k: list(v) for k, v in merged_dynamic_fields.items()}
            
#             # Step 3: Update db_dict with the merged fields for each tab
#             for tab, fields in merged_dynamic_fields.items():
#                 if tab not in db_dict:
#                     db_dict[tab] = {}                       # Initialize if not already present
#                 for field in fields:
#                     db_dict[tab].setdefault(field, '')      # Set the field with an empty string as its value, if not already set
    
#     logging.info(F"req_fields before req {req_fields}")
...
#         logging.info(req_fi, out_dic, db_dict)
...
#         logging.info(out_dic)
#         logging.info(F"\n")
#         db_dict[div_tab] = out_dic

#     logging.info(F"db_dict is {db_dict}")
...
#             parsed_fields.update(extracted_field)
#             unused_fields.update(unused_field)
#             logging.info(f"Final Extracted fields are {parsed_fields}")
#         except Exception as e:
...
#         if not cus_date_df.empty:
#             cus_date_df.columns = [col.lower() for col in cus_date_df.columns]
#             cus_date_df = cus_date_df.to_dict(orient='records')
#             for data in cus_date_df:
#                 for k, v in data.items():
#                     if v is None or v == "":
#                         unused_fields[k] = ""
#                     else:
#                         parsed_fields[k] = f"E:{v}"
#         ## update all the parsed field percentage etc at once into the db in a separte thread
#         executor.submit(calculate_ext_percentage, parsed_fields, unused_fields, tenant_id, case_id)
#         # final_extracted_fields=calculate_ext_percentage(parsed_fields,unused_fields, tenant_id,case_id)
#     except Exception as e:
#         logging.exception(f"Exception Occurred while calculating percentage .. {e}")
#         pass
...
#     # Connect to the extraction database
#     extraction_db = DB('extraction', **db_config)
#     logging.info(F" ####### adding no of fileds extracted into field accuarcy table")
...
#             logging.info(f"Final Extracted fields are {parsed_fields}")
#         except Exception as e:
#             logging.exception(f"Exception occured while parsing fields..{e}")
...
#         # final_extracted_fields=calculate_ext_percentage(parsed_fields,unused_fields, tenant_id,case_id)
#     except Exception as e:
#         logging.exception(f"Exception Occured while calculating percentage .. {e}")
#         pass
...
def get_highlight_n_table(update_ocr_data,predict_page,ocr_data_word_all,seg):
    try:
        ...
        logging.info(f" ################ error occurred in fn get_highlight_n_table. Error is {e}")
        return {}
    ...
    for key, values in update_ocr_data.items():
            ...
            for index,pred in enumerate(values):
                for col,head_dict in pred.items():
                    for head,value_dict in head_dict.items():
                        try:
                            logging.info(f"predict_page is {predict_page,col}")
                            start_page = int(new_page_predict[col.lower()])
                        except:
                            start_page = int(new_page_predict[head.lower()])
                        ocr_data_word = ocr_data_word_all[start_page]
...
def get_training_data(case_id, tenant_id):
    ...
    # CUSTOMER_NAME = extraction.execute_(query)['party_id'].to_list()[0]
    ...
    if party_id:
        ...
        try:
            PRIORITY_DATA = json.loads(data['PRIORITY_DATA'].to_list()[0]) if not data['PRIORITY_DATA'].empty else {}
        except Exception as e:
            logging.info(f"Error parsing PRIORITY_DATA: {e}")
            PRIORITY_DATA = {}

        try:
            PRIORITY_CONTEXT = json.loads(data['PRIORITY_CONTEXT'].to_list()[0]) if not data['PRIORITY_CONTEXT'].empty else {}
        except Exception as e:
            logging.info(f"Error parsing PRIORITY_CONTEXT: {e}")
            PRIORITY_CONTEXT = {}

        try:
            PRIORITY_FORMULA = json.loads(data['PRIORITY_FORMULA'].to_list()[0]) if not data['PRIORITY_FORMULA'].empty else {}
        except Exception as e:
            logging.info(f"Error parsing PRIORITY_FORMULA: {e}")
            PRIORITY_FORMULA = {}

        try:
            USER_TRAINED_FIELDS = json.loads(data['USER_TRAINED_FIELDS'].to_list()[0]) if data['USER_TRAINED_FIELDS'].to_list()[0] else {}
            # USER_TRAINED_FIELDS = json.loads(data['USER_TRAINED_FIELDS'].to_list()[0]) if not data['USER_TRAINED_FIELDS'].empty else {}
        except Exception as e:
            logging.info(f"Error parsing USER_TRAINED_FIELDS: {e}")
            USER_TRAINED_FIELDS = {}

        try:
            TRAINED_DATE_LIST = data['TRAINED_DATE'].to_list()
            TRAINED_DATE = TRAINED_DATE_LIST[0] if TRAINED_DATE_LIST else ''
        except Exception as e:
            logging.info(f"Error getting TRAINED_DATE: {e}")
            TRAINED_DATE = ''

    return PRIORITY_CONTEXT, PRIORITY_DATA, PRIORITY_FORMULA, USER_TRAINED_FIELDS, TRAINED_DATE


#present fucntion working13_10_2025
# def get_training_data(case_id,tenant_id):
...
#                 line.remove(key) 
#             if line:
#                 diff_ver=abs(abs((key["top"]+key["bottom"])/2 -(line[0]["top"]+line[0]["bottom"])/2))
#                 dif=abs(diff_ver-thr)
#                 if min>dif:
#                     logging.info(F" here diff for values are {min  , dif}")
#                     next_values=[]
#                     min=dif
#                     for word in line:
#                         if k_right<word['left']:
#                             next_values.append(word)
#     logging.info(f"  we have for next values as {next_values} where thr is {thr}")
#     if min>5:
...
# def find_next_ele_crop(key, ocr_data, thr):
#     """
#     Enhanced next element finder:
#       - Uses adaptive thresholds
#       - Auto-retries other directions if below fails
#       - Handles multi-line OCR blocks
#     """
#     exceeded_min_flag = False
#     next_values = []
#     sorted_lines = line_wise_ocr_data(ocr_data)
#     k_right = key["right"]
#     min_diff = 9999
#     thr_y = _adaptive_threshold([ocr_data])
...
#         diff_ver = abs(((key["top"] + key["bottom"]) / 2) - ((line[0]["top"] + line[0]["bottom"]) / 2))
#         dif = abs(diff_ver - thr)
#         if dif < min_diff:
#             logging.info(f"[Enhance] diff check: min={min_diff:.2f}, cur={dif:.2f}")
#             next_values = [w for w in line if w["left"] > k_right - 3]  # allow slight overlap
#             min_diff = dif

#     logging.info(f"[Enhance] Next values (below)={next_values}, thr={thr:.2f}")

#     # retry other directions if below yields nothing
#     if not next_values:
#         logging.info("[Enhance] Retrying directions since no values found below...")
#         directions = ["above", "right", "left"]
#         for direction in directions:
#             alt_vals = _search_alternate_direction(key, ocr_data, direction, thr_y)
#             if alt_vals:
#                 next_values = alt_vals
#                 logging.info(f"[Enhance] Found next values in direction={direction}")
#                 break

#     if min_diff > 5:
#         exceeded_min_flag = True

#     return next_values, exceeded_min_flag, min_diff
# # trail alternative 29_10_2025555555
# def find_next_ele_crop(key, ocr_data, thr):
#     """
#     Enhanced next element finder:
#       - Prioritizes numeric values
#       - Uses adaptive thresholds
#       - Auto-retries other directions if below fails
#       - Handles multi-line OCR blocks
#     """
...
#     # --- First pass: check below direction (default)
#     for line in sorted_lines:
#         if key in line:
#             line.remove(key)
#         if not line:
...
#         dif = abs(diff_ver - thr)
#         if dif < min_diff:
#             next_values = [w for w in line if w["left"] > k_right - 3]
#             min_diff = dif
...
#     if not next_values:
#         logging.info("[Enhance] Retrying directions since no values found below...")
#         directions = ["right", "above", "left"]

#         for direction in directions:
#             # Relaxed threshold for horizontal scanning (helps 'Total Stocks Value')
#             alt_thr = thr_y * (1.5 if direction in ["right", "left"] else 1)
#             alt_vals = _search_alternate_direction(key, ocr_data, direction, alt_thr)

#             if alt_vals:
#                 print(f"the altvals are:{alt_vals}")
#                 numeric_alt = [v for v in alt_vals if re.search(r'\d', v.get("word", ""))]
#                 print(f"the numeric alts are:::{numeric_alt}")
#                 if numeric_alt:
#                     next_values = numeric_alt
#                     logging.info(f"[Enhance] Found numeric next values in direction={direction} (relaxed thr={alt_thr:.2f})")
#                     break
#                 else:
#                     logging.info(f"[Enhance] Found non-numeric values in {direction}, skipping")
...
#     #         alt_vals = _search_alternate_direction(key, ocr_data, direction, thr_y)
#     #         if alt_vals:
#     #             # Prefer numeric entries from the alternate direction
#     #             numeric_alt = [v for v in alt_vals if re.search(r'\d', v.get("word", ""))]
#     #             if numeric_alt:
#     #                 next_values = numeric_alt
#     #                 logging.info(f"[Enhance] Found numeric next values in direction={direction}")
#     #                 break
#     #             else:
#     #                 logging.info(f"[Enhance] Found non-numeric values in {direction}, skipping")

#     # --- Apply final numeric sanity filter
#     next_values = [v for v in next_values if re.search(r'\d', v.get("word", ""))]
#     if min_diff > 5:
#         exceeded_min_flag = True
...
def find_next_ele_crop(key, ocr_data, thr):
    ...
    #  New adaptive fallback (safe, non-breaking)
    #  New adaptive fallback (safe, non-breaking)
    try:
        print("try;;;;;;;;")
        if not next_values:
            logging.info("[AdaptiveFix] No next_values found — retrying with boosted threshold")

            # Increase threshold adaptively (handle larger spacing between header and next line)
            boosted_thr = max(thr * 3, thr_y * 2)
            print(f"the boosted thr is:::::{boosted_thr}")

            # Fallback candidate collector
            fallback_candidates = []

            for line in sorted_lines:
                # calculate distance between header key and line center
                diff_ver = ((line[0]["top"] + line[0]["bottom"]) / 2) - ((key["top"] + key["bottom"]) / 2)

                # only consider lines below header (positive diff)
                if diff_ver > 0 and diff_ver < boosted_thr:
                    # numeric pattern match including comma numbers (1,23,456 or 12.45)
                    candidate_vals = [
                        w for w in line
                        if w["left"] > k_right - 3 and re.search(r'\d', w["word"])
                    ]
                    if candidate_vals:
                        fallback_candidates.extend(candidate_vals)
                        print(f"[AdaptiveFix] line candidates -> {candidate_vals}")

            # sort by vertical order to pick the closest below
            if fallback_candidates:
                fallback_candidates = sorted(fallback_candidates, key=lambda x: x["top"])
                next_values = fallback_candidates[:1]  # pick closest below header
                print(f"[AdaptiveFix] Selected fallback next_values={next_values}")
                logging.info(f"[AdaptiveFix] Found fallback next_values={next_values}")
            else:
                # Extra wide search — for cases like 'Total Stocks Value' where the number is much lower
                logging.info("[AdaptiveFix] Fallback empty — expanding search window")
                for line in sorted_lines:
                    if ((line[0]["top"] + line[0]["bottom"]) / 2) > key["bottom"]:
                        for w in line:
                            if w["left"] > k_right - 3 and re.search(r'\d', w["word"].replace(',', '')):
                                next_values = [w]
                                logging.info(f"[AdaptiveFix] Found deep fallback={w}")
                                print(f"[AdaptiveFix] Deep fallback={w}")
                                break
                        if next_values:
                            break
    except Exception as e:
        logging.warning(f"[AdaptiveFix] fallback failed silently: {e}")


    # try:
    #     print("try;;;;;;;;")
    #     if not next_values:
    #         # if no match found, retry once with adaptive vertical threshold
    #         logging.info("[AdaptiveFix] No next_values found — retrying with boosted threshold")
    #         boosted_thr = max(thr, thr_y * 1.2)
    #         print(f"the boosted thr is:::::{boosted_thr}")
    #         for line in sorted_lines:
    #             diff_ver = abs(((key["top"] + key["bottom"]) / 2) - ((line[0]["top"] + line[0]["bottom"]) / 2))
    #             if diff_ver < boosted_thr:
    #                 print("iffffff")
    #                 candidate_vals = [w for w in line if w["left"] > k_right - 3 and re.search(r'\d', w["word"])]
    #                 if candidate_vals:
    #                     print(f"the condit vals are:::{candidate_vals}")
    #                     next_values = candidate_vals
    #                     logging.info(f"[AdaptiveFix] Found fallback next_values={next_values}")
    #                     break
    # except Exception as e:
    #     logging.warning(f"[AdaptiveFix] fallback failed silently: {e}")
...
def _search_alternate_direction(key, ocr_data, direction, thr):
    """Helper to scan other directions generically."""
    next_vals = []
    if direction == "above":
        next_vals = [w for w in ocr_data if w["bottom"] < key["top"] and abs(key["top"] - w["bottom"]) < thr]
    elif direction == "right":
        next_vals = [w for w in ocr_data if w["left"] > key["right"] and abs(w["top"] - key["top"]) < thr]
    elif direction == "left":
        next_vals = [w for w in ocr_data if w["right"] < key["left"] and abs(w["top"] - key["top"]) < thr]
    return sorted(next_vals, key=lambda x: x.get("left", 0))



def find_table_crop_header(each_value,ocr_data,header,col_f):
    print(f"the header is::::{header}")
    print(f"the each_value is::::{each_value}")
    print(f"the col_f is::::{col_f}")
    ...
    logging.info(f" ################ sorted_list_head got is {sorted_list_head}")
    row_head_l=[]
    row_head=''
            ...
            if not t_fl:
                if abs(item['bottom']-row_top)<=(2*abs(item['top']-item['bottom'])):
                    if col_f['right']<item['left']:
                        row_head_l.append(item['word'])
                        row_top=item['top']
        # logging.info(F"row_head_l is {row_head_l}")
    ...
    logging.info(F" ############ hea and row_h are {hea,row_h}")
    matcher = SequenceMatcher(None, row_h, hea)
    similarity_ratio_head = matcher.ratio()
    logging.info(F' ################ final row head got is {row_head} and similarity_ratio_head is {similarity_ratio_head}')
...
#         tem=1000
#         logging.info(F" ################ con_dict_poss amd col_dict_poss is {con_dict_poss} and {col_dict_poss}")
...
#                     logging.info(F"dista is {dista} and distance is {distance}")
#                     tem=abs(distance-dista)
#                     tem_col=[item1,tem,item2]
#         if tem_col:
#             out_col.append(tem_col)

#     logging.info(f" ################ column possibilites got are {out_col}")
...
#         cont_dict={}
#     logging.info(f" ################ finalised column is {column_dict}")
#     if flag:
#         return column_dict
#     else:
...
def _normalize_word(word):
    """Clean and normalize OCR words."""
    return re.sub(r'[^a-zA-Z0-9]', '', str(word)).lower().strip()
...
def _fuzzy_ratio(a, b):
    """Compute simple similarity score using difflib only."""
    if not a or not b:
        return 0
    a, b = _normalize_word(a), _normalize_word(b)
    return SequenceMatcher(None, a, b).ratio()


def _adaptive_threshold(ocr_data_all):
    """Derive dynamic thresholds from OCR heights."""
    if not ocr_data_all:
        return 20
    heights = [w['height'] for page in ocr_data_all for w in page if 'height' in w]
    if not heights:
        return 20
    avg_h = sum(heights) / len(heights)
    return max(2.5 * avg_h, 20)
...
def get_col(col, cont, ocr_data_all, distance, flag=True):
    """
    Enhanced get_col:
      - Fuzzy matching for both context and column
      - Adaptive distance threshold
      - Automatic fallback if column/context not found
    """
    out_col = []
    temp_col_max, temp_con_max = 0, 0
    max_match_column_dict, max_match_con_dict = {}, {}
    col_ = _normalize_word(col)
    print(f"the col is::::{col_}")
    cont_ = _normalize_word(cont.get('word', ''))
    print(f"the cont is::::::{cont_}")

    thr_y = _adaptive_threshold(ocr_data_all)
    logging.info(f"[Enhance] Adaptive threshold for matching: {thr_y:.2f}")

    for ocr_data in ocr_data_all:
        tem_col = []
        con_dict_poss, col_dict_poss = [], []
        for word in ocr_data:
            w_word = _normalize_word(word['word'])

            # --- Column matching ---
            sim_col = _fuzzy_ratio(col_, w_word)
            if sim_col > 0.82:
                if sim_col > temp_col_max:
                    temp_col_max = sim_col
                    max_match_column_dict = word
                col_dict_poss.append(word)

            # --- Context matching ---
            sim_con = _fuzzy_ratio(cont_, w_word)
            if sim_con > 0.82:
                if sim_con > temp_con_max:
                    temp_con_max = sim_con
                    max_match_con_dict = word
                con_dict_poss.append(word)
        ...
        logging.info(f"[Enhance] con_dict_poss={con_dict_poss} | col_dict_poss={col_dict_poss}")

        # fallback logic when either is missing
        if not col_dict_poss and max_match_column_dict:
            col_dict_poss.append(max_match_column_dict)
        if not con_dict_poss and max_match_con_dict:
            con_dict_poss.append(max_match_con_dict)

        # final fallback: create synthetic column if still empty
        if not col_dict_poss and con_dict_poss:
            ctx = con_dict_poss[0]
            fake_col = ctx.copy()
            fake_col['top'] += thr_y
            fake_col['bottom'] += thr_y * 1.5
            col_dict_poss = [fake_col]
            logging.info("[Enhance] Created synthetic column candidate due to missing header")
        ...
        if con_dict_poss and col_dict_poss:
            for item1 in col_dict_poss:
                for item2 in con_dict_poss:
                    dista = calculate_distance(item1, item2)
                    diff = abs(distance - dista)
                    tem_col = [item1, diff, item2]
                    out_col.append(tem_col)
                    logging.info(f"[Enhance] Dist={dista:.2f}, Target={distance:.2f}, Diff={diff:.2f}")

    logging.info(f"[Enhance] column possibilities={out_col}")
    ...
    if out_col:
        ...
        column_dict = max_match_column_dict or {}
        cont_dict = max_match_con_dict or {}

    logging.info(f"[Enhance] Final column={column_dict}")
    return (column_dict if flag else (column_dict, cont_dict))
...
def get_crop_values(col,head,cont,ocr_data_all):
    ...
    logging.info(f" ################ \n")
    logging.info(f" ################ \n")
    logging.info(f" ################ col head cont send to get_crop_values are {col,head,cont}")
    logging.info(f" ################ \n")
    logging.info(f" ################ \n")
    logging.info(f" ################ \n")
    ...
    logging.info(f" ################ next_values got is {next_values}")
    ...
    logging.info(f"################ numerical_values after filtering {numerical_values}")
    out_val={}
    thr=0
    if head:
        for value in numerical_values:
            ...
            logging.info(f" ################ head is {head} for value is {value}")
            if head_ and similarity_ratio_head>thr:
                out_val=value
                thr=similarity_ratio_head

        logging.info(f" ################ value got is {out_val} ")
        if out_val:
            return out_val
        else:
    ...
    if not head and (not numerical_values or excceded_min_flag):
        logging.info(F" ############## no head and no numbeers or excedded flag {excceded_min_flag} for  {column_dict}")
...
def priority_func(seg,predicted,case_id,tenant_id,ocr_data_all,predict_page,rec_flag=''):
    print(f"the prediced is :::::::::{predicted}")
    prio_context,prio_data,prio_formula,user_trained_fields,date=get_training_data(case_id,tenant_id)
    logging.info(f" ################ prio_data got is  {prio_data}")
    logging.info(f" ################ prio_context is {prio_context}")
    logging.info(f" ################ prio_context is {prio_formula}")
    logging.info(f" ################ user_trained_fields is {user_trained_fields}")
    ...
    croped_high={}
    logging.info(F' ################ starting priority function')
    final_pairs={}
    out_col=[]
    label_col={}
    pri_col_head={}
    ...
    for key in prio_data:
        if key not in predicted:
            ...
            predicted[key]=[{}]
    print(f"the prediced are::::::::{predicted}")
    for label, values in predicted.items():
        logging.info(F" ################ label is {label}")
        temp_col=[]
        for pred in values:
            for col,head_dict in pred.items():
                temp_col.append(col)
        if label in prio_data and not rec_flag:
            logging.info(F" ################ label present in prio_data")
            for temp_ou,val_ou in prio_data[label].items():
                if not isinstance(prio_data[label], dict):
                    print(f"[FIX] {label} stored as string, converting to empty dict")
                    prio_data[label] = {}

                if '_crop' in temp_ou or values == [{}]:
                    if label not in crop_fields:
                        crop_fields[label]=[]
                    logging.info(F" ################ label is field cropped")
                    ou=temp_ou
                    crop_fields[label].append(ou.split('_')[0]+"~"+val_ou)

                elif temp_ou in temp_col:
                    logging.info(F" ################ column is present in predicted cols")
                    ou=temp_ou
                    pri_col_head[ou]=val_ou
                else:
                    logging.info(F" ################ not a croped or found in the predicted cols")
                    if temp_col:
            ...
            out_col.append(ou)
            label_col[ou]=label
    logging.info(f" ################ out_col are column finalised are {out_col}")
    temp_head_all={} 
    head_val={}
    for label, values in predicted.items():
        for pred in values:
    ...
    logging.info(f" ################ headers for each col is temp_head_all:{temp_head_all} ")
    logging.info(F" ################ headers to values mapped dict is head_val:{head_val}")
    ...
    logging.info(f" ################ final_pairs for col and header are {final_pairs}")
    logging.info(f" ################ final_pairs for crop_fields are {crop_fields}")
    logging.info(f" ################ predicted are {predicted}")
    final_pred={}
    training={}
    for label, values in predicted.items():
        temp={}
        ...
        if label in crop_fields:
            ...
            for crop_field in crop_fields[label]:
                ...
                logging.info(F" ################ label is in cropped fileds {label}")
                ...
                try:
                    ...
                    if te_head:
                        try:
                            ...
                            logging.info(f" ################ crop_value is {crop_value} for te_head {te_head} and te_col {te_col}")
                            if crop_value:
                        ...
                        logging.info(F"for non header head an dvalues git are {crop_temp} and {croped_high}")
                except Exception as e:
                    logging.info(f" ##################### there is some error here with {crop_field} and error is {e}")
                    continue
            if not temp:
                logging.info(f" ################ te for rec is {te}")
                rec_pred[label].append(te)
                logging.info(f" ################ rec_pred for rec is {rec_pred}")
                if label in prio_formula:
                    trained_frmls=prio_formula[label]
                else:
                    trained_frmls=[]
                logging.info(f" ################ trained_frmls is {trained_frmls}")
                if trained_frmls:
                    opt_frml,val=get_opt_frm(trained_frmls,all_label_vals)
                    if val:
                        temp[te_col+'_'+te_head]=[opt_frml]
                        logging.info(f" ################ trained_frmls output is  {val} and formula is {opt_frml}")
                    else:
                        temp,other=priority_func(seg,rec_pred,case_id,tenant_id,ocr_data_all,predict_page,True)
                        logging.info(f" ################ trained_frmls failed so called rec outs are {other} and {temp}")
                elif len(all_label_vals) >= 1:
                    logging.info(f" ################ all_label_vals is {all_label_vals}")
                    temp[list(all_label_vals.keys())[0]]=[list(all_label_vals.values())[0]]
                # else:
                #     # try:
                #     #     temp,other=priority_func(seg,rec_pred,case_id,tenant_id,ocr_data_all,predict_page,True)
                #     #     logging.info(f" ################ no trained_frmls so called rec outs are {other} and {temp}")
                #     # except:
                #     continue
        ...
        if label in final_pairs:
            for pred in values:
                for col,head_dict in pred.items():
                    for head,value in head_dict.items():
                        logging.info(F" ################ label is in non cropped fileds {label}")
...
def find_values_n(seg,word_list,desired_label,col='col'):
    print(f" recived list and lable for find_values here added is is {word_list} -------------> {desired_label}")
    print(f"the col is::::::::::::{col}")

    # logging.info(f" recived list and lable for find_values is {word_list} -------------> {desired_label}")
    ...
    try:
        ...
        logging.info(f"######################### joblib file for this seg is missing {vectorizer_filename} and {model_filename}")
        return {}
...
#             logging.info(f"the word in find values word is {word} and highest_confidence is {highest_confidence}")
    ...
    logging.info(f"output got from value (find_values) svm is {best_word} for {desired_label}")
    return best_word
...
def update_tarining_data(PRIORITY_DATA, PRIORITY_CONTEXT, case_id, tenant_id):
    ...
    CUSTOMER_NAME = extraction_db.execute_(query)['customer_name'].to_list()[0]
    logging.info(f"CUSTOMER_NAME got is {CUSTOMER_NAME}")
    queryyy = f"SELECT `party_id` from  `ocr` where `case_id` = '{case_id}'"
    party_id = extraction_db.execute_(queryyy)['party_id'].to_list()[0]
    logging.info(f"party_id got is {party_id}")
    if  party_id:
        print(f"the party_id isssss::::::::{party_id}")
        try:
            # Check if record already exists for this party_id
            query = f"SELECT * FROM `trained_info` WHERE `party_id` = '{party_id}'"
            data = template_db.execute_(query)
            logging.info(f"Fetched existing record for party_id {party_id}: {data}")
            ...
            if data.empty:
                ...
                logging.info(f"Existing record found for party_id {party_id}. Updating training data.")
...
# ####this is code having present 13_10_2025
# def update_tarining_data(PRIORITY_DATA,PRIORITY_CONTEXT,case_id,tenant_id):
#     db_config['tenant_id']=tenant_id
#     temaplate_db=DB('template_db',**db_config)
#     extraction_db=DB('extraction',**db_config)
...
#         CUSTOMER_NAME = CUSTOMER_NAME.replace("LIMITED", "").strip()
#         try:
#             query = f"SELECT  * from  `trained_info` where `CUSTOMER_NAME` = '{CUSTOMER_NAME}'"
...
def date_match(date):
    ...
    # Find and logging.info dates in each sentence
...
def date_extract(document_id_df_all,date_trained):
    if not isinstance(date_trained, dict):
        return [], ''
    ...
    for words in document_id_df_all:
        ...
        # # Logging.info the words grouped by horizontal lines
...
def non_standard_docs(case_id,tenant_id,seg):
    ...
    prio_context,prio_data,prio_formula,user_trained_fields,date=get_training_data(case_id,tenant_id)
    logging.info(F"date got is {date}")
    if date:
        extraction=DB('extraction',**db_config)
        max_cond=0
        date_extrac=''
        ...
        for words in document_id_df_all:
            ...
            for line in line_groups:
                ...
                similarity_ratio_col = matcher.ratio()
                logging.info(line_words,line_f,similarity_ratio_col,max_cond)
                if max_cond<similarity_ratio_col:
                    max_cond=similarity_ratio_col
                    date_extrac = line_words
                    line_f=line
        logging.info(max_cond,line_f,date_extrac)
        date_valid=True
        if max_cond<0.5:
        ...
        try:
            ...
            logging.info(F" ################## out_words is {out_words}")
            out_words['word']=date_extrac
            date_high=from_high_cor(out_words)
            logging.info(F" ################## date_high is {date_high}")
            update_highlights(tenant_id,case_id,{"date_stat":date_high})
        except:
            pass

        logging.info(F"date_extract got is {date_extrac}")
    ...
    for i,document_id_df in enumerate(document_id_df_all):
        try:
            ...
            logging.info(f"  ################ non_stand_date predicted is {out__}")
    ...
    logging.info(F" ################ ")
    logging.info(F" ################ ")
    logging.info(f" ################ final_out got is  {final_out}")
    logging.info(f" ################ head is {head}")
    logging.info(f" ################ Sec_head is {Sec_head}")
    logging.info(f" ################ dates are {dates}")
    logging.info(f" ################ page_no_dict is {page_no_dict}")
    ...
    logging.info(f" ################ level_2_keys_list is {level_2_keys_list}")
    logging.info(F" ################ ")
    logging.info(F" ################ ")

    #here we will send all the best keys that we finalised to this function for further extraction process
    logging.info(f" ################ entering into find_non_standard_values")
    ...
    logging.info(f" ################ predicted values are {predicted}")
...
def find_table_user_trained_header(ocr_data,headers,cont,head_dis):
    ...
    logging.info(f" ################ header got is {headers}")
  
    head_list=[]
    for word in ocr_data:
        for head in headers:
            logging.info(f" ############## head is {head} and word is {word['word']}")
    ...
    logging.info(f" ############ here Grouped headers are {grouped_headers}")
    ...
    for group in grouped_headers:
        ...
        logging.info(f" ############ here group_join has minimum thr are  {group_join}")
...
def select_best_value(candidates):
    """Prefer numeric or amount-like tokens."""
    if not candidates:
        return None

    # Step 1: prioritize numeric-like tokens
    numeric_candidates = [c for c in candidates if re.search(r'\d', c['word'])]
    print(f"the num cad iares:::{numeric_candidates}")
    if numeric_candidates:
        # optional: prioritize currency/amount-like (with comma or decimal)
        currency_like = [c for c in numeric_candidates if re.search(r'[\d,]+\.\d+', c['word'])]
        if currency_like:
            return currency_like[0]
        print(f"the numeric values is::::{numeric_candidates}")
        return numeric_candidates[0]

    # Step 2: fallback to first token
    return candidates[0]
...
def extract_user_trained_fields(ocr_data_all,user_trained_fields):
    ...
    for predict,trained_dict in user_trained_fields.items():
        ...
        if not cont or not head or not col:
            logging.info(f" #################### insuffieent user trained data for {predict}")
            continue
        
        column_dict,context=get_col(col,cont,ocr_data_all,distance,False)

        logging.info(f" ########### here we got column and context {column_dict}  {context}")
        ...
        logging.info(f"################ header_box after  {header_box}")

        if header_box:
            max_calculated_diff=10000
            predicted={}
            for value in numerical_values:
                filtered_value=re.sub(r'[^0-9,.]', '', str(column_dict))
                ...
                if not enhanced_value:
                    ...
                    logging.info(f"################ calculate_value_distance is  {diff_value_header} and {diff_value_head}")
                calculated_diff=abs(diff_value_head-diff_value_header)
                logging.info(f"################ calculated_diff is  {calculated_diff} for {value}")
                if max_calculated_diff>calculated_diff:
                    max_calculated_diff=calculated_diff
                    predicted=value
            logging.info(f"################ final predict is  {predicted}")
        ...
        #  NEW FIX APPLIED HERE (Fallback when header not found)
        else:
            logging.info(f"No valid header for {predict}, trying fallback")
            if numerical_values:
                print(f"the numericv alues are:::{numerical_values}")
                # use smart numeric-first selection
                final_candidate = select_best_value(numerical_values)
                print(f"te final coordinate s are::::::{final_candidate}")
                if final_candidate:
                    predicted_fields[predict] = final_candidate['word']
                    high[predict] = from_high_cor(final_candidate)
            elif excceded_min_flag or not next_values:
                high[predict] = from_high_cor(column_dict)
                predicted_fields[predict] = re.sub(r'[^0-9,.-]', '', column_dict['word'])   
                         
        #newly added else conditions
        # else:
        #     # MINIMAL FIX 3: When header not found, try to extract from next_values or column
        #     logging.info(f"No valid header for {predict}, trying fallback")
        #     if numerical_values:
        #         predicted_fields[predict] = numerical_values[0]['word']
        #         high[predict] = from_high_cor(numerical_values[0])
        #     elif excceded_min_flag or not next_values:
        #         high[predict] = from_high_cor(column_dict)
        #         predicted_fields[predict] = re.sub(r'[^0-9,.-]', '', column_dict['word'])
    return predicted_fields,high
    
def remap_predicted_fields(predicted_fields, case_id, tenant_id,extraction_db):
    """
    Convert OCR predicted field keys using AGE_MARGIN_WORKING_UAT and COMPONENT_MASTER.
    Save mapping dictionary into trained_info.extra_variables.
    """
    
    # 1. Fetch PARTY_ID from OCR
    qry = f"SELECT party_id FROM ocr WHERE case_id='{case_id}'"
    result = extraction_db.execute_(qry).to_dict(orient='records')
    if not result:
        logging.warning("No PARTY_ID found for case")
        return predicted_fields
    
    PARTY_ID = result[0]["party_id"]
    print(f"the party_id is::::::{PARTY_ID}")
    # 2. Prepare mapping dictionary
    new_predicted = {}
    mapping_dict = {}   # { new_key : old_key }
    print(f"the predicted fields is:::::::{predicted_fields}")
    for old_key, value in predicted_fields.items():
        
        qry = f"""
                    SELECT A.component_name,
            A.is_active,
            C.status
        FROM   age_margin_working_uat A
            LEFT JOIN component_master C
                    ON Cast(A.component_name AS VARCHAR2(4000)) = Cast(
                        C.component_name AS VARCHAR2(4000))
        WHERE  A.party_id = '{PARTY_ID}'
            AND Lower(Cast(A.component_name AS VARCHAR2(4000))) LIKE Lower(
                '%{old_key}%')
            AND A.is_active = 1
            AND Cast(C.status AS VARCHAR2(4000)) = 'ACTIVE' 

        """
        print(f"the query is:::::::{qry}")
        age_res = extraction_db.execute_(qry).to_dict(orient='records')
        print(f"the ages res is::::::::{age_res}")
        
        if not age_res:
            # Not found - keep same key
            new_predicted[old_key] = value
            continue
        age_res=age_res[0]['component_name']
        mapping_dict[age_res] = old_key
        new_predicted[age_res] = value
        print(f"the new procrted is::::::::{new_predicted}")    
    

    # 3. Save mapping_dict into trained_info.extra_variables
    print(f"the mapping dict is:::{mapping_dict}")
    try:
        md_json = json.dumps(mapping_dict)
        up_sql = f"""
            UPDATE ocr 
            SET extra_variables='{md_json}'
            WHERE party_id='{PARTY_ID}'
        """
        extraction_db.execute_(up_sql)
        logging.info(f"Stored extra_variables mapping: {mapping_dict}")
    except Exception as e:
        logging.error(f"Error saving extra_variables: {e}")
    print(f"the new rpedicted is::::::::::{new_predicted}")
    return new_predicted,mapping_dict
...
def prio_funtion():
    ...
    if not seg:
        ...
        logging.info(F" ################ predict_page is {predict_page}")
        logging.info(F" ################ high_dict is {high_dict}")
        logging.info(F" ################ predicted is {predicted}")
        logging.info(F" ################ column_head_dict is {column_head_dict}")

        #priority function
        logging.info(F" ################ starting the priority_func")
        ...
        logging.info(F" ################ after priority function output for predicted_priority is {predicted_priority} and training is {training}")

        logging.info(F" ################ user_trained_fields are {user_trained_fields}")
        user_trained_high={}
        ...
        logging.info(F" ################ user_trained_predicted_fields are {user_trained_predicted_fields}")
        if flag:
            ...
            logging.info(F" ############### final predicted_context for all predicted fields is {predicted_context}")

            #function that store data in tempalte db
            logging.info(F" ################ flag is True so upadating the training data")
        ...
        try:
            logging.info(F" ################ upadating into database")
            try:
                print("---------------------")
                logging.info("### remapping OCR keys based on AGE_MARGIN & COMPONENT_MASTER")
                predicted_priority,mapping_dict= remap_predicted_fields(predicted_priority, case_id, tenant_id,extraction_db)
            except Exception as e:
                print(f"Exception: {e}")   
            print(f"here for he prio function:::####")     
            update_db_ntable(predicted_priority,case_id,tenant_id,mapping_dict)
        except Exception as e:
            logging.info(F" ################ Exception is {e}")

        #here this fucntion generate the highlights for the predcited values
        logging.info(F" ################ getting highlights")
        high=get_highlight_n_table(high_dict,predict_page,document_id_df_all,seg)
        logging.info(F" ################ croped_high are {croped_high}")
        ...
        for key,val in high.items():
            if key in mapping_dict:
                new_high[mapping_dict[key]]=val
            else:
                new_high[key]=val

        logging.info(F"  ################ final high_dict for all predicted fields is {new_high}")
        #we will update the generated highlights to the database
        update_highlights(tenant_id,case_id,new_high)

    else:
        logging.info(F"  ################ Standard format and seg is {seg}")
...
def get_divivsion_labels(word_list,seg):
    ...
    try:
        ...
        logging.info(f"######################### joblib file for this seg is missing {model_filename} and {vectorizer_filename}")
        return {}
    ...
    output={}
    # logging.info the predictions for each word
    
    for word, prediction in zip(word_list, predictions):
#         logging.info(f"Word: {word}, Predicted Label: {prediction}")
        if prediction !='other':
...
def has_numeric_word(data_list):
    ...
    for data_dict in data_list:
        ...
        if 'word' in data_dict and any(char.isdigit() for char in data_dict['word']):
            logging.info(F'retuning true for the {data_dict}')
...
def get_sub_labels(word_list, seg):
    ...
    try:
        ...
        logging.info(f"Joblib files for segment {seg} are missing: {model_filename} and {vectorizer_filename}")
        return {}
...
def predict_sub_divisions(predcit,ocr_word,seg):
    ...
    logging.info(f"divisions sub div is {division} \n")
    if not division:
        return {}
    list_=[]
    for word in ocr_word:
    ...
    for i in range(0,len(list_)):
        # logging.info(f'i is {i} and list is {list_[i]}')
        if i==len(list_)-1:
            bottom_word={}
            table_bottom=1000
        else:
        ...
        temp=[]
        # logging.info(F'top and bottom are {table_top} ,{table_bottom}')
...
def predict_table_divisions(ocr_words, words, seg):
    ...
    logging.info(f"Divisions obtained from logistic regression: {division}\n")
    ...
    for i in range(len(list_)):
        if flag == 1:
            logging.info(F"Skipping this iteration")
            flag = 0
            continue
...
def predict_table_headers(word_list, div, seg):
    ...
    try:
        ...
        logging.info(f"Joblib file for this division is missing: {vectorizer_filename}, {model_filename}")
        return {}
...
def find_table_header(each_value, ocr_data, header, predict='', seg='', api=True):
    ...
    # logging.info the results for debugging purposes
    logging.info(f"Result found is {return_list} and head is {head}")
...
def enhance_headers(headers,word_div):
    ...
    logging.info(f"startring enhancing headers")
    ...
    logging.info(f"field_list got is {headers}")
    ...
    right=[]
    for word in word_div:
        logging.info(f"for the word {word['word']}")
        if word['word'] in field:
            logging.info(F"int the if")
            dif_line=abs(word['left']-field_line)
            logging.info(F"int the field_line {field_line}  word['left'] {word['left']} dif_line is{dif_line}")
            if dif_line< 100:
                ...
                logging.info(F"int the dif_top {dif_top} word['top'] {word['top']} head_line {head_line}")
                if dif_top <= 15:
                    out_list['header_keywords'].append(word['word'])
                    sort_list[word['word']]=word['left']
                else:
                    out_words.append(word)
            logging.info(f"out_list is {out_list}")
        elif word['word'] in header:
            logging.info(f"in the else")
...
def remove_from_ocr(remove_words,ocr_data_parse):
    ...
    for ocr_data in ocr_data_parse:
        if remove_words != ocr_data['word']:
            # logging.info(f"#### matched word is {word}m###### text is {ocr_data['word']}")
...
def get_head_alias(table_headers,seg):
    ...
    """
    logging.info(f"header send  is {table_headers}")
    value=''
    for key in table_headers:
        ...
        if len(values) == 2:
            ...
            if 'more' in key or "greater" in key:
                ...
                value=("<"+str(values[0]))
    logging.info(f"returing value is {value}")
    return value
...
def main_2d(ocr_data,field_keywords,predcit,seg):
    ...
    ocr_data_=ocr_data
    if field and header:
        logging.info(f"entered the if of main 2d")
        for key in field:
            for data in ocr_data:
                word=data['word']
                if key==word:
                    ...
                    logging.info(F"nexvalues got is {next_values}")
                    try:
                        if next_values:
                    ...
                    for each_value in next_values:
                        ...
                        logging.info(f"table_header got is {table_header} for each value {each_value['word']}")
        ...
        logging.info("forming custom table")
        custom_table=custom_1(predict_values)

    elif header:
        logging.info(f"entered the else of main 2d")
        for key in header:
            for data in ocr_data:
                word=data['word']
                if key==word:
                    ...
                    logging.info(next_values)
                    next_values=filter_num(next_values)
                    logging.info(f"numerical_values got is {next_values}")
                    if next_values:
        ...
        logging.info("forming the custom table")
...
def get_col_range(input_string,row_head=''):
    ...
    try:
        ...
        logging.info(F"returing is {input_string}")                  
        return input_string                 
    except Exception as e:
        logging.info(f'exceptionis {e}')
        return ''
...
def get_db_labels(word_list,seg,div):
    ...
    try:
        ...
        logging.info(F"file not found {vectorizer_filename,model_filename}")
        return ''
...
def update_into_db(predicted_fields,case_id,tenant_id,seg,pred):
    ...
    for key,values in predicted_fields.items():
            for value in values:
                for col,val in value.items():
                    ...
                    logging.info(f"get_db_labels column got is {column}")

                    col_head=get_col_range(col)
                    logging.info(f"range got is {col_head}")
                    if column:
                        if not col_head:
                            head=get_db_labels([col],seg,"head")
                            logging.info(f"get_db_labels head got is {head}")
    ...
    if column:
        ...
        extraction_db.execute_(sql)
        logging.info(F" ####### final out_dict to store in the db is {column}")
        ...
        extraction_db.execute_(sql)
        logging.info(F" ####### final out_dict to store in the db is {predicted_fields}")
...
#     logging.info(corpus)
    ...
    for key,value in predicted_fields.items():
        ...
        logging.info(f"cleaned_string are {cleaned_string} ")
...
def get_highlight_t(update_ocr_data,high_map,start_page,ocr_data_word,predcit,removed_word_map,multi_add):
    ...
    logging.info(f"data got is {update_ocr_data , high_map , start_page  , predcit , removed_word_map , multi_add}")
    logging.info(f"removed_word_map got is {removed_word_map}")
    value_highlights={}
    try:
        for key,value_ in update_ocr_data.items():
            if key in multi_add:
                ...
                values=[value_]
            logging.info(F"values is {values}")
            for value__ in values:
                temp={}
                for words in ocr_data_word:
                    value=value__
                    if value==words['word']:
                        logging.info(f"word for which highlighit needed  is {value}")
                        row_head=high_map[key][1]
                        col_head=high_map[key][0]
                        logging.info(f"word for which row_head col_head  is{row_head} and {col_head}")
                        out=find_table_header(words,ocr_data_word,[row_head],[])
                        before_list=find_before_ele(words,ocr_data_word)
                        logging.info(f"row head got is {out} and column head got is {before_list}")
        ...
        logging.info(f"error occured in fn get_highlight_t error is {e}")
        pass
...
def get_highlight_n_t(update_ocr_data,start_page,ocr_data_word,seg):
    ...
    try:
        ...
        logging.info(f"error occured in fn get_highlight_n_t error is {e}")
        return {}
...
def find_table_highlight(ocr_data,row_head,col_head,value,start_page):
    ...
    logging.info(f"data got is {row_head , col_head , value ,start_page}")
    ...
    with zipkin_span(
        ...
        try:
            ...
            try:
                logging.info(F" ################ ")
                logging.info(f"F ################ ")
                logging.info(f"F ################ strating the prediction for non_tabular fields")
                logging.info(f"F ################ ")
                logging.info(F" ################ ")
                if start_page==0:
                    ...
                    logging.info(f" ####### Response Received from cetroid_api is {response_}")
                    ...
                    # logging.info(f"non_table_fields  got is {non_fields}")

                    
                    # update_db_nf(non_fields,case_id,tenant_id,seg)
            except Exception as e:
                logging.exception(e)
                logging.info(f"########## error in exttraction of non_table fields")
            ...
            if seg:
                for document_id_df in document_id_df_all:
                    ...
                    logging.info(f"word_list got is {word_list}")
                    
                    logging.info(f"strating the table_fields prediction \n")
                    if word_list:
                        divisions=predict_table_divisions(document_id_df,word_list,seg)
                    logging.info(f"divisions recived from predict_table_divisions {divisions}\n")
                    ...
                    for predcit,word_div__ in divisions.items(): 
                        for word_div_ in word_div__:
                            if not word_div_:
                                continue
                            logging.info(f"the prediction staring for divsion is {predcit} \n")

                            logging.info(f"divisions send to enhance_divisions_ver is {word_div_} \n")

                            logging.info(f"divisions send to enhance_divisions_ver is {word_div_} \n")
                            # word_div_=enhance_divisions_ver(word_div_,predcit,seg)
                            word_div_=[word_div_]
                            logging.info(f"divisions got from enhance_divisions_ver is {word_div_} \n")
                            ...
                            for word_div in word_div_:
                                ...
                                logging.info(f"field_keywords got from predict_table_headers is {field_keywords} \n")

                                if field_keywords:

                                    if field_keywords['header_keywords']:
                                        logging.info(f" headers sent to table header check is {field_keywords['header_keywords'],word_div} \n")
                                        word_div,field_keywords['header_keywords']=check_headers(field_keywords['header_keywords'],word_div)
                                        logging.info(f"headers got from table header check is {word_div}  and {field_keywords['header_keywords']}\n")
                                    if field_keywords['field_keywords'] and field_keywords['header_keywords'] :
                                        ...
                                        logging.info(f"sort_list is {sort_list}\n")
                                    else:
                                        sort_list={}

                                    logging.info(f"field_keywords got from enhance_headers is {field_keywords}\n")
                                    predicted_fields,removed_word_map,final_table=main_2d(word_div,field_keywords,predcit,seg)
                                    
                                    logging.info(f"output from predicted_fields is {predicted_fields}\n")
                                    ...
                                    try:
                                        if predicted_fields:
                                            ...
                                            try:    
                                                ...
                                                logging.info(F"table highlight got is {table_high}")
                                            except:
                                                table_high={}
                                                pass
                                            ...
                                            logging.info(F"highlight got is {high_} and map is {high_map}")
                                            if high_:
                                        ...
                                        logging.exception(e)
                                        logging.info(f"########## unable to upadate into db")
                        
                    start_page=start_page+1
            if not updated or not seg:
                logging.info(F" ################ ")
                logging.info(F" ################ ")
                logging.info(f" ################ starting non standard predcition ")
                logging.info(F" ################ ")
                logging.info(F" ################ ")
                response_non_stand=non_standard_docs(case_id,tenant_id,seg)
                logging.info(F" ################ ")
                logging.info(F" ################ ")
                logging.info(f" ################ done with non standard predcition ")
                logging.info(F" ################ ")
                logging.info(F" ################ ")
...
def custom_table_data():
    ...
    """
    data = request.json
    logging.info(f"requested data {data}")
...
def update_into_db_table(predicted_fields,tenant_id,case_id,seg,pred):
    ...
    for key,values in predicted_fields.items():
        for col,val in values.items():
                logging.info(f"get_db_labels sent is {key}")
                column=get_db_labels([key],seg,"column")
                logging.info(f"get_db_labels column got is {column}")
                key_ = re.sub(r'[^<>a-zA-Z0-9]', '', pred+column)
                key_= key_.lower()
                logging.info(f" range sent to col_finder {key_,col}")
                ...
                logging.info(F"\n")
    logging.info(F" #######    out_dict to before sum is {out_dict} and {highlight}")
    logging.info(F" #######    multi_add got is  {multi_add}")
    remove=[]
    ...
    logging.info(F" #######    out_dict to store in the db is {out_dict} and {highlight}")
    
    logging.info(F" #######    out_dict to store in the db is {out_dict}")
    temp=pred+'_OCR'
    logging.info(F" ####### final out_dict to store in the db is {out_dict}")
        ...
        for field_list in json.loads(query_data):
            ...
            for field in field_list:
                if field in out_dict:
                    logging.info(field_list[0],field,out_dict)
    ...
    logging.info(F" ####### after addition of missing cols out_dict to store in the db is {out_dict}")
...
def field_accuracy_(ui_dict,tenant_id,case_id,column):
    ...
    if query_data:
        ...
        logging.info(f"changed are {changed_all}") 
...
def save_table_ocr():
    ...
    try:
        ...
        logging.info(f" ############ table_data is {table_val} and table is {table}")
        try:
            if table_val:
                table_dict = {}
                ...
                logging.info(f" ############ table_dict is {table_dict}")

                if table and table_dict:
                    high_light=update_into_db_table(table_dict,tenant_id,case_id,seg,table)
                else:
                    logging.info(f" ############ table_dict not updated table_dict is {table_dict} table is {table}")
        except Exception as e:
            logging.exception("Error in custom_table function", e)
            pass
